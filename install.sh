...
apt update -y && apt install curl wget gnupg git dos2unix psmisc zip unzip nginx -y > /dev/null 2>&1
...
# INFO, PROFIL, GAME, RIWAYAT, RIWAYAT TOPUP, MUTASI MINIFIED (TETAP SAMA SEPERTI V68)
...
while true; do clear
    ...
    echo -e "  ${GREEN}10.${NC} Setup Domain (Nginx + Cloudflare)"
    echo ""
    ...
    case $choice in
        ...
        10)
            clear
            echo -e "${CYAN}===============================================${NC}"
            echo -e "${YELLOW}            🌐 SETUP DOMAIN (NGINX)            ${NC}"
            echo -e "${CYAN}===============================================${NC}"
            echo -e "Pastikan Domain/Subdomain Anda sudah di-pointing"
            echo -e "ke IP VPS ini melalui Cloudflare (A Record)."
            echo -e "Di Cloudflare, atur SSL/TLS ke mode 'Flexible' atau 'Full'."
            echo ""
            read -p "Masukkan Nama Domain (contoh: web.domain.com): " domain_name
            if [ ! -z "$domain_name" ]; then
                echo -e "\n${YELLOW}Menginstal Nginx...${NC}"
...
EOFNGINX
                ...
                echo -e "Silakan akses website melalui: http://$domain_name"
            else
