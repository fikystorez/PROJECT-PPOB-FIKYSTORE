...
echo "=========================================================="
echo "  MENGINSTAL DIGITAL FIKY STORE - V46 (CUSTOM ALERT FIX)  "
echo "=========================================================="
...
# HTML PROVIDER (FIX ALERT CUSTOM)
cat << 'EOF' > public/provider.html
<!DOCTYPE html>
<html lang="id" id="html-root">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pilih Provider - DIGITAL FIKY STORE</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <script>tailwind.config = { darkMode: 'class' }</script>
</head>
<body class="bg-gray-50 dark:bg-gray-900 font-sans transition-colors duration-300">
    <div class="max-w-md mx-auto bg-[#f4f6f9] dark:bg-gray-900 min-h-screen relative pb-8 shadow-2xl overflow-x-hidden">
        
        <div class="flex items-center p-4 bg-[#001229] text-white shadow-md sticky top-0 z-40">
            <i class="fas fa-arrow-left text-xl cursor-pointer text-gray-300 hover:text-white mr-4 px-2" onclick="history.back()"></i>
            <h1 class="font-bold text-[17px] tracking-wide flex-1" id="pageTitle">Pilih Provider</h1>
        </div>

        <div class="px-4 mt-6 mb-2">
            <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider ml-1">Pilih Operator Tujuan</p>
        </div>

        <div class="mx-4 bg-white dark:bg-[#0f172a] rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
    ...
    <div id="customAlert" class="fixed inset-0 z-[1001] hidden flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-[#0f172a] rounded-[1.5rem] p-6 w-full max-w-[320px] text-center shadow-[0_10px_40px_rgba(0,0,0,0.5)] border border-gray-100 dark:border-gray-700 transform transition-transform scale-100">
            <div id="alertIcon" class="mb-4 text-6xl"></div>
            <h3 class="text-xl font-bold text-[#001229] dark:text-white mb-2 tracking-wide" id="alertTitle">Pemberitahuan</h3>
            <p class="text-sm text-gray-500 dark:text-gray-300 mb-6" id="alertMessage">Pesan</p>
            <button onclick="closeAlert()" class="bg-[#001229] dark:bg-[#facc15] text-yellow-400 dark:text-[#0f172a] w-full py-3 rounded-xl font-bold tracking-widest shadow-md hover:bg-[#002147] dark:hover:bg-yellow-500 transition">OKE</button>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('pageTitle').innerText = type === 'data' ? 'Paket Internet' : (type === 'pulsa' ? 'Isi Pulsa' : 'Pilih Provider');

        // Modal Logic
        let alertCallback = null;
        function showAlert(title, msg, isSuccess, cb) { 
            document.getElementById('alertTitle').innerText = title; 
            document.getElementById('alertMessage').innerText = msg; 
            // Ikon ganti jadi tools (gear) biar sesuai dengan "Segera Hadir"
            document.getElementById('alertIcon').innerHTML = isSuccess ? '<i class="fas fa-check text-green-500"></i>' : '<i class="fas fa-tools text-yellow-500"></i>'; 
        ...
        // Provider Data
        const providers = [
            { name: 'Axis', color: 'bg-purple-600', initial: 'AX' },
            { name: 'Indosat', color: 'bg-yellow-500', initial: 'IS' },
            { name: 'Smartfren', color: 'bg-red-500', initial: 'SF' },
            { name: 'Telkomsel', color: 'bg-red-600', initial: 'TS' },
            { name: 'By.U', color: 'bg-blue-400', initial: 'BU' },
            { name: 'Three', color: 'bg-gray-800', initial: '3' },
            { name: 'XL', color: 'bg-blue-600', initial: 'XL' }
        ];

        let html = '';
        providers.forEach((p, idx) => {
            const border = idx !== providers.length - 1 ? 'border-b border-gray-100 dark:border-gray-800' : '';
            // Ganti default alert() dengan showAlert() custom UI
            html += `
            <div class="flex items-center px-5 py-4 ${border} cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition" onclick="showAlert('Segera Hadir!', 'Menu nominal untuk ${p.name} sedang dalam tahap pengembangan.', false)">
                <div class="w-10 h-10 rounded-full ${p.color} text-white flex items-center justify-center font-extrabold text-sm shadow-sm mr-4 tracking-tighter">${p.initial}</div>
                <div class="flex-1 font-bold text-[#001229] dark:text-gray-200 text-[15px] tracking-wide">${p.name}</div>
...
# HTML DASHBOARD (Hanya menyertakan sisanya tanpa diubah logikanya agar rapi)
...
<body class="bg-gray-50 dark:bg-gray-900 font-sans transition-colors duration-300">
    ...
    <script>
        ...
        function toggleDarkMode() { isDark = !isDark; localStorage.setItem('darkMode', isDark); applyDarkMode(); } applyDarkMode();
        fetch('/api/banners').then(res => res.json()).then(data => { if(data.banners && data.banners.length > 0 && data.banners[0] !== "") { const slider = document.getElementById('promoSlider'); const dotsContainer = document.getElementById('promoDots'); slider.innerHTML = data.banners.map((url) => `<div class="w-full h-full shrink-0 snap-center relative flex items-center justify-center bg-[#002147]"><img src="${url}" class="absolute inset-0 w-full h-full object-cover"></div>`).join(''); dotsContainer.innerHTML = data.banners.map((_, i) => `<div class="w-2 h-2 rounded-full bg-white opacity-${i===0?'100':'40'} transition-opacity duration-300 dot-indicator shadow-sm"></div>`).join(''); } const el = document.getElementById('promoSlider'); let dots = document.querySelectorAll('.dot-indicator'); let cur = 0; el.addEventListener('scroll', () => { let idx = Math.round(el.scrollLeft / el.clientWidth); dots.forEach((d, i) => { d.classList.toggle('opacity-100', i === idx); d.classList.toggle('opacity-40', i !== idx); }); cur = idx; }); setInterval(() => { cur = (cur + 1) % dots.length; el.scrollTo({ left: cur * el.clientWidth, behavior: 'smooth' }); }, 3500); });
        function bantuanWA() { const adminNumber = "6282231154407"; const text = `Halo Admin, saya butuh bantuan terkait aplikasi Digital Fiky Store.\n\nNama: ${user.name}\nNo HP: ${user.phone}`; window.open(`https://wa.me/${adminNumber}?text=${encodeURIComponent(text)}`, '_blank'); }
        let selectedMethod = ''; function openTopupModal() { document.getElementById('topupModal').classList.remove('hidden'); setTimeout(() => document.getElementById('topupModalContent').classList.add('modal-enter'), 10); } function closeTopupModal() { document.getElementById('topupModalContent').classList.remove('modal-enter'); setTimeout(() => document.getElementById('topupModal').classList.add('hidden'), 300); } function setNominal(val) { document.getElementById('topupNominal').value = val; }
        function selectMethod(method) { selectedMethod = method; document.querySelectorAll('.method-label').forEach(el => el.classList.remove('border-yellow-400', 'bg-yellow-50', 'dark:bg-gray-800')); document.querySelectorAll('.radio-indicator').forEach(el => el.classList.remove('border-yellow-400')); document.querySelectorAll('.inner-dot').forEach(el => el.classList.add('hidden')); document.getElementById('lbl-' + method).classList.add('border-yellow-400', 'bg-yellow-50', 'dark:bg-gray-800'); document.getElementById('radio-' + method).classList.add('border-yellow-400'); document.querySelector(`#radio-${method} .inner-dot`).classList.remove('hidden'); }
        ...
        function processTopup() { const nom = document.getElementById('topupNominal').value; if(!nom || nom < 10000) return showAlert('Peringatan', 'Minimal Top Up adalah Rp 10.000', false); if(!selectedMethod) return showAlert('Peringatan', 'Silakan pilih metode pembayaran', false); if(selectedMethod === 'manual') { const adminWA = "6282231154407"; const text = `Halo Admin, saya ingin *Top Up Saldo* (Manual).\n\nNama: ${user.name}\nNo Akun: ${user.phone}\nNominal: *Rp ${parseInt(nom).toLocaleString('id-ID')}*\n\nMohon instruksi pembayarannya.`; closeTopupModal(); window.open(`https://wa.me/${adminWA}?text=${encodeURIComponent(text)}`, '_blank'); } else if (selectedMethod === 'qris') { closeTopupModal(); document.getElementById('qrisAmountDisplay').innerText = 'Rp ' + parseInt(nom).toLocaleString('id-ID'); document.getElementById('qrisModal').classList.remove('hidden'); } }
        function closeQrisModal() { document.getElementById('qrisModal').classList.add('hidden'); } function confirmQris() { closeQrisModal(); showAlert('Diproses', 'Sistem sedang mengecek pembayaran QRIS Anda secara otomatis. Saldo akan bertambah jika sukses.', true); }
    </script>
</body>
</html>
EOF
...
# ==========================================
# FILE HALAMAN PROFILE, RIWAYAT, INFO (TETAP AMAN)
# ==========================================
...
# ==========================================
# FILE NODE.JS (TETAP SAMA SEPERTI SEBELUMNYA)
# ==========================================
echo "[4/5] Memastikan layanan Backend berjalan..."
cat << 'EOF' > index.js
...
app.post('/api/auth/register', async (req, res) => {
    const { name, phone, email, password } = req.body; let webUsers = loadJSON(webUsersFile);
    ...
    webUsers[fPhone] = { name, email, password, isVerified: false, otp, photo: '' }; saveJSON(webUsersFile, webUsers);
...
app.post('/api/auth/verify', (req, res) => {
    const { phone, otp } = req.body; let users = loadJSON(webUsersFile); let fPhone = phone.toString().replace(/[^0-9]/g, ''); if (fPhone.startsWith('0')) fPhone = '62' + fPhone.slice(1);
    if (users[fPhone] && users[fPhone].otp === otp.toString().trim()) {
...
app.post('/api/auth/login', (req, res) => {
    const { identifier, password } = req.body; let users = loadJSON(webUsersFile); let fPhone = identifier.toString().replace(/[^0-9]/g, ''); if (fPhone.startsWith('0')) fPhone = '62' + fPhone.slice(1);
...
app.post('/api/auth/forgot', async (req, res) => {
    const { phone } = req.body; let users = loadJSON(webUsersFile); let fPhone = phone.toString().replace(/[^0-9]/g, ''); if (fPhone.startsWith('0')) fPhone = '62' + fPhone.slice(1);
    if (!users[fPhone]) return res.status(400).json({ error: 'Nomor HP tidak terdaftar.' });
    const otp = Math.floor(1000 + Math.random() * 9000).toString(); users[fPhone].otp = otp; saveJSON(webUsersFile, users);
    const sent = await sendWhatsAppMessage(fPhone, `Kode OTP Reset Password: *${otp}*`);
    if(sent) res.json({ message: 'OTP Terkirim', phone: fPhone }); else res.status(500).json({ error: 'Gagal mengirim OTP.' });
});

app.post('/api/auth/reset', (req, res) => {
    const { phone, otp, newPassword } = req.body; let users = loadJSON(webUsersFile); let fPhone = phone.toString().replace(/[^0-9]/g, ''); if (fPhone.startsWith('0')) fPhone = '62' + fPhone.slice(1);
    if (users[fPhone] && users[fPhone].otp === otp.toString().trim()) {
        users[fPhone].password = newPassword; users[fPhone].otp = null; saveJSON(webUsersFile, users); res.json({ message: 'Password berhasil diubah!' });
    } else { res.status(400).json({ error: 'Kode OTP Salah.' }); }
});

app.post('/api/user/update', async (req, res) => {
    const { oldPhone, name, newPhone, photo, otp } = req.body; let users = loadJSON(webUsersFile); let db = loadJSON(dbFile);
    ...
    if (fOld !== fNew) {
        ...
        if (!otp) {
            const genOtp = Math.floor(1000 + Math.random() * 9000).toString(); users[fOld].updateOtp = genOtp; saveJSON(webUsersFile, users);
            ...
            users[fNew] = { ...users[fOld], name: name, photo: photo !== undefined ? photo : users[fOld].photo }; delete users[fNew].updateOtp; delete users[fOld];
            if (db[fOld]) { db[fNew] = { ...db[fOld] }; delete db[fOld]; }
        }
    } else { users[fOld].name = name; if (photo !== undefined) users[fOld].photo = photo; }
...
app.post('/api/user/change-password', async (req, res) => {
    const { phone, oldPassword, newPassword, otp } = req.body; let users = loadJSON(webUsersFile); let fPhone = phone.toString().replace(/[^0-9]/g, ''); if (fPhone.startsWith('0')) fPhone = '62' + fPhone.slice(1);
    if (!users[fPhone] || users[fPhone].password !== oldPassword) return res.status(400).json({ error: 'Password lama salah.' });

    if (!otp) {
        const genOtp = Math.floor(1000 + Math.random() * 9000).toString(); users[fPhone].passOtp = genOtp; saveJSON(webUsersFile, users);
        ...
        users[fPhone].password = newPassword; delete users[fPhone].passOtp; saveJSON(webUsersFile, users); res.json({ message: 'Password berhasil diubah!' });
    }
});

app.post('/api/user/delete', (req, res) => {
    const { phone } = req.body; let users = loadJSON(webUsersFile); let db = loadJSON(dbFile);
    let fPhone = phone.toString().replace(/[^0-9]/g, ''); if (fPhone.startsWith('0')) fPhone = '62' + fPhone.slice(1);
    if (users[fPhone]) delete users[fPhone]; if (db[fPhone]) delete db[fPhone];
    saveJSON(webUsersFile, users); saveJSON(dbFile, db); res.json({ message: 'Akun dihapus' });
});
...
async function startBot() {
    ...
    sock.ev.on('connection.update', (update) => { const { connection } = update; if(connection === 'close') { startBot(); } else if(connection === 'open') { console.log("✅ BOT WHATSAPP TERHUBUNG!"); } });
...
while true; do
    clear
    echo -e "${C}${B}╔═══════════════════════════════════════════════════╗${N}"
    echo -e "${C}${B}║${N} ${Y}⚡ DIGITAL FIKY STORE - VPS CONTROL PANEL (V46) ⚡${N} ${C}${B}║${N}"
    echo -e "${C}${B}╠═══════════════════════════════════════════════════╣${N}"
    ...
    echo -e "${C}${B}║${N}  ${G}8.${N} 📢 Kelola Pusat Informasi (Info)              ${C}${B}║${N}"
    echo -e "${C}${B}╠═══════════════════════════════════════════════════╣${N}"
    ...
    echo -e "${C}${B}╚═══════════════════════════════════════════════════╝${N}"
    echo -e ""
    read -p "${B}Pilih menu [0-8]: ${N}" choice
    ...
    case $choice in
        ...
        6)
            ...
            node -e "
                ...
                console.log('\n✅ Foto banner diperbarui! (Restart Menu 2)');
            "
            read -p "Tekan Enter..." ;;
        8)
            clear
            echo "=== MANAJEMEN PUSAT INFORMASI ==="
            echo "1. Tambah Informasi Baru"
            echo "2. Hapus Semua Informasi"
            echo "0. Kembali"
            read -p "Pilih [0-2]: " info_choice
            case $info_choice in
                1)
                    read -p "Judul Info: " judul
                    read -p "Isi / Deskripsi: " isi
                    node -e "
                        const fs = require('fs');
                        const file = '$HOME/$DIR_NAME/info.json';
                        let infos = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : [];
                        const tgl = new Date().toLocaleDateString('id-ID', {day:'numeric', month:'short', year:'numeric'});
                        infos.push({ title: \`$judul\`, content: \`$isi\`, date: tgl });
                        fs.writeFileSync(file, JSON.stringify(infos, null, 2));
                        console.log('✅ Info berhasil ditambahkan dan langsung tampil di Web!');
                    "
                    read -p "Tekan Enter..." ;;
                2)
                    node -e "const fs=require('fs'); fs.writeFileSync('$HOME/$DIR_NAME/info.json', '[]'); console.log('✅ Semua info berhasil dihapus!');"
                    read -p "Tekan Enter..." ;;
                *) ;;
            esac
            ;;
