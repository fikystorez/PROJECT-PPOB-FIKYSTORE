...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
    ...
    <script>
        ...
        // V108: AUTO-DETECT PREFIX NUMBER
        const prefixMap = {
        ...
        document.getElementById('inputTarget').addEventListener('input', function() {
            ...
            if(val.length >= 4) {
                ...
                if(found) { prefObj.innerText = found; prefObj.classList.remove('hidden'); } 
            ...
            try {
                ...
                if(data.data && data.data.length > 0) {
                    let html = data.data.map(p => {
                        ...
                        let clickAction = p.is_open ? `showProductDetail('${safeSku}', '${safeName}', ${p.price}, ${p.isLocal}, '${safeDesc}')` : `Swal.fire({icon:'error', title:'Gangguan', text:'Produk sedang gangguan server.', background: '#0b1320', color: '#fff'})`;
                        ...
                        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-800 cursor-pointer hover:bg-gray-50 dark:hover:bg-[#1a2639] transition ${opacityClass}" onclick="${clickAction}">
                            <div class="flex flex-col pr-4 w-2/3">
                                <span class="text-[12px] font-bold text-gray-800 dark:text-gray-100 leading-tight">${p.name} ${p.isLocal ? '<i class="fas fa-check-circle text-green-500 text-[10px]" title="Produk Lokal"></i>' : ''}</span>
                            </div>
        ...
        // V108: MODAL DETAIL BOTTOM SHEET
        ...
        function bantuanAdmin() {
            window.open(`https://wa.me/6282231154407?text=`+encodeURIComponent(`Halo Admin, mau tanya produk *${selectedName}* aman kan?`), '_blank');
        }
        ...
        function executeBuy() {
            ...
            setTimeout(() => {
                ...
                fetch('/api/transaction/create', {
                    ...
                    if (resp.ok) Swal.fire({icon: 'success', title: 'Berhasil!', text: data.message}).then(()=>{ window.location.href = '/riwayat.html'; });
                    else Swal.fire({icon: 'error', title: 'Gagal', text: data.error});
                }).catch(err => Swal.fire({icon: 'error', title: 'Oops!', text: 'Gangguan jaringan.'}));
            }, 300);
        }
        ...
        function selectProvider(op) {
            ...
            if(provider) {
                ...
                if (provider.items && provider.items.length > 0 && type === 'data') {
                    ...
                    titleEl.innerText = provider.name;
                    document.getElementById('inputTarget').placeholder = provider.placeholder || "Ketik target...";
        ...
        function selectCategory(catName, placeholderStr) {
            ...
            document.getElementById('inputTarget').placeholder = placeholderStr || "Ketik target...";
            document.getElementById('inputTarget').value = ''; 
            document.getElementById('inputTarget').dispatchEvent(new Event('input')); 
        ...
        function goBack() {
            if(currentState === 'product') {
                if(currentProvider && currentProvider.items && currentProvider.items.length > 0 && type === 'data') {
                    ...
                    currentState = 'operator'; document.getElementById('productContainer').classList.add('hidden'); document.getElementById('operatorContainer').classList.replace('hidden', 'block'); titleEl.innerText = originalTitle;
                }
...
# ==========================================
# FILE NODE.JS (API + AUTO-CHECKER + AUTO-BACKUP)
# ==========================================
...
// V108: BACKGROUND AUTO BACKUP TELEGRAM
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}          💎 PANEL DIGITAL FIKY STORE (V108) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
    ...
    echo ""
    echo -e "${PURPLE}[ 🛡️ BACKUP & RESTORE CLOUD ]${NC}"
    ...
    case $choice in
        ...
        20)
            ...
            read -p "Enter..." ;;
        21) 
            echo -e "${RED}Pastikan Anda sudah mem-push script V108 ke GitHub Anda!${NC}"
...
pm2 restart all > /dev/null 2>&1
echo "  SISTEM WEB V108 BERHASIL DIPERBARUI SECARA PENUH!       "
