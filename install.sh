...
echo -e "${C_YELLOW}${C_BOLD}  MENGINSTAL DIGITAL FIKY STORE - V51 (FIX PANEL VPS)     ${C_RST}"
echo -e "${C_CYAN}${C_BOLD}==========================================================${C_RST}"
...
echo "[2/5] Membuat direktori aplikasi..."
...
const fs = require('fs'); const crypto = require('crypto'); const ALGO = 'aes-256-cbc'; const KEY = crypto.scryptSync('DigitalFikyStore_SecureKey_2026', 'salt', 32);
function encrypt(text) { let iv = crypto.randomBytes(16); let cipher = crypto.createCipheriv(ALGO, KEY, iv); let encrypted = cipher.update(text); encrypted = Buffer.concat([encrypted, cipher.final()]); return iv.toString('hex') + ':' + encrypted.toString('hex'); }
function decrypt(text) { let textParts = text.split(':'); let iv = Buffer.from(textParts.shift(), 'hex'); let encryptedText = Buffer.from(textParts.join(':'), 'hex'); let decipher = crypto.createDecipheriv(ALGO, KEY, iv); let decrypted = decipher.update(encryptedText); decrypted = Buffer.concat([decrypted, decipher.final()]); return decrypted.toString(); }
module.exports = { load: (file, defaultData = {}) => { try { if (!fs.existsSync(file)) return defaultData; let raw = fs.readFileSync(file, 'utf8'); if(!raw) return defaultData; if (raw.trim().startsWith('{') || raw.trim().startsWith('[')) { let parsed = JSON.parse(raw); module.exports.save(file, parsed); return parsed; } return JSON.parse(decrypt(raw)); } catch(e) { return defaultData; } }, save: (file, data) => { fs.writeFileSync(file, encrypt(JSON.stringify(data, null, 2))); } };
EOF
...
# File HTML dipampatkan agar muat di script ini
    ...
    while true; do
        ...
        case $backchoice in
            1)
                ...
                zip backup.zip config.json database.json info.json ssl_backup.tar.gz 2>/dev/null
        ...
        case $prodchoice in
            1)
                ...
                node -e "
                    ...
                    produk[key] = { nama: process.env.TMP_NAMA, harga: parseInt(process.env.TMP_HARGA), deskripsi: process.env.TMP_DESC, kategori: catName, brand: brandName };
            ...
            2)
                ...
                node -e "
                    const crypt = require('./fiky_crypt.js'); let produk = crypt.load('produk.json'); let keys = Object.keys(produk);
                ...
                eval $(node -e "
                    ...
                    if(isNaN(idx) || idx < 0 || idx >= keys.length) { console.log('export VALID=false'); } else {
                        let k = keys[idx]; let p = produk[k]; console.log('export VALID=true');
                        console.log('export OLD_KODE=\"' + k + '\"'); console.log('export OLD_NAMA=\"' + p.nama.replace(/[\"$\\\\]/g, '\\\\$&') + '\"'); console.log('export OLD_HARGA=\"' + p.harga + '\"');
                    }
                ")
                ...
                node -e "
                    const crypt = require('./fiky_crypt.js'); let produk = crypt.load('produk.json');
                    let oldKey = process.env.OLD_KODE; let newKey = process.env.NEW_KODE.toUpperCase().replace(/\s+/g, '');
                    ...
                    if (process.env.NEW_DESC && process.env.NEW_DESC.trim() !== '') { if (process.env.NEW_DESC.trim() === '-') delete item.deskripsi; else item.deskripsi = process.env.NEW_DESC; }
                    if(!item.brand) item.brand = 'Lainnya';
                    if (oldKey !== newKey) { produk[newKey] = item; delete produk[oldKey]; } else { produk[oldKey] = item; }
                    crypt.save('produk.json', produk); console.log('\x1b[32m\n✅ Perubahan pada produk berhasil disimpan!\x1b[0m');
                "
                read -p "Tekan Enter untuk kembali..."
                ;;
            3)
                ...
                if [ ! -f "$nama_file_excel" ]; then
                    ...
                    node -e "
                        ...
                        try {
                            ...
                            rawData.forEach(row => {
                                let keys = Object.keys(row); let getColStrict = (keywords) => keys.find(k => keywords.includes(k.toLowerCase().trim()));
                                ...
                                let descCol = getColStrict(['desc', 'deskripsi']); let brandCol = getColStrict(['brand', 'provider', 'operator']);
    ...
    case $choice in
        ...
        10)
            ...
            if [ "$reset_sesi" == "y" ]; then
                ...
                echo -e "${C_GREEN}✅ Sesi berhasil dihapus. Silakan jalankan bot kembali (Menu 1) untuk menautkan nomor baru.${C_RST}"
            fi
            read -p "Tekan Enter untuk kembali..."
            ;;
        11)
            ...
            node -e "
                ...
                crypt.save('info.json', infos); console.log('\x1b[32m✅ Info berhasil ditambahkan dan langsung tampil di Web!\x1b[0m');
            "
            read -p "Tekan Enter untuk kembali..."
            ;;
        12)
...
echo "=========================================================="
echo "  SUKSES V51 ULTIMATE! Silakan ketik 'menu' di terminal."
echo "=========================================================="
