...
# ==========================================
# FILE NODE.JS (API TRANSAKSI FIX CRASH DATABASE)
# ==========================================
...
// API TRANSAKSI (V92 FIX DATABASE CRASH)
app.post('/api/transaction/create', async (req, res) => {
    try {
        ...
        // AUTO-HEAL: Pastikan array riwayat sudah dibuat untuk member lama
        ...
        if (!isLocal && config.digiUser && config.digiKey) {
            try {
                ...
                let isDev = (config.digiKey || '').toLowerCase().includes('dev');

                let digiRes = await axios.post('https://api.digiflazz.com/v1/transaction', {
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}           💎 PANEL DIGITAL FIKY STORE (V92) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
