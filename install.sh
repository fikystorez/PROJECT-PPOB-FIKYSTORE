...
echo "=========================================================="
echo "   MENGINSTAL DIGITAL FIKY STORE V113 (ULTIMATE UNCUT)    "
echo "=========================================================="
...
.centered-modal-box { background-color: #002147; padding: 2.5rem 1.5rem 2rem 1.5rem; border-radius: 1.2rem; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.3), 0 10px 10px -5px rgba(0, 0, 0, 0.2); width: 90%; max-width: 360px; text-align: center; position: relative; z-index: 10; margin: auto; margin-top: 10vh;}
...
<body class="flex flex-col items-center justify-center min-h-screen relative bg-[#fde047]">
        ...
        document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (res.ok) { 
                    ...
                    Swal.fire({ icon: 'success', title: 'Aman!', text: 'Silakan cek WA Anda untuk kode OTP.', background: '#002147', color: '#fff' });
                } else { 
        ...
        document.getElementById('otpLoginForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (res.ok) { 
                    ...
                    Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Login Sukses.', background: '#002147', color: '#fff', timer: 1500, showConfirmButton: false }).then(() => {
    ...
    <script>
        ...
        document.getElementById('registerForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                Swal.fire({ icon: 'error', title: 'Oops...', text: 'Gagal memproses pendaftaran.', background: '#002147', color: '#fff' }); 
            }
        });
        ...
        document.getElementById('otpForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const data = await res.json();
                if (res.ok) { 
                    Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Akun terverifikasi. Silakan Login.', background: '#002147', color: '#fff' }).then(() => { 
                ...
                Swal.fire({ icon: 'error', title: 'Oops...', text: 'Gagal verifikasi OTP.', background: '#002147', color: '#fff' }); 
            }
        });
    </script>
</body>
...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
    <div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden transition-colors duration-300">
        ...
        <div class="fixed bottom-0 w-full max-w-md bg-white dark:bg-[#001229] border-t border-gray-200 dark:border-gray-800 flex justify-around p-3 pb-4 shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.1)] dark:shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.5)] z-40 transition-colors">
            ...
            <div class="flex flex-col items-center cursor-pointer text-gray-400 hover:text-[#002147] dark:hover:text-yellow-400 transition" onclick="location.href='/riwayat.html'">
                <i class="fas fa-file-alt text-xl"></i><span class="text-[10px] mt-1 font-bold">RIWAYAT</span>
            </div>
            <div class="flex flex-col items-center cursor-pointer text-gray-400 hover:text-[#002147] dark:hover:text-yellow-400 transition" onclick="location.href='/info.html'">
                <i class="fas fa-bell text-xl"></i><span class="text-[10px] mt-1 font-bold">INFO</span>
            </div>
            <div class="flex flex-col items-center cursor-pointer text-gray-400 hover:text-[#002147] dark:hover:text-yellow-400 transition" onclick="location.href='/profile.html'">
    ...
    <script>
        ...
        .then(res => res.json()).then(data => { 
            document.getElementById('headTrx').innerText = data.transactions.length + ' Trx';
        });
...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
        ...
        <div class="flex-1 overflow-y-auto hide-scrollbar pb-10">
            <div id="operatorContainer" class="block">
                <div class="px-4 mt-6">
                    ...
                    <div class="bg-white dark:bg-[#111c2e] rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-800 shadow-sm transition-colors" id="opListRender"></div>
                </div>
            </div>
            ...
            <div id="categoryContainer" class="hidden">
                ...
                <div class="bg-white dark:bg-[#111c2e] shadow-sm transition-colors pb-4" id="categoryList"></div>
            </div>
            ...
            <div id="productContainer" class="hidden">
                ...
                <div class="bg-white dark:bg-[#111c2e] shadow-sm transition-colors pb-4" id="productList"></div>
            </div>
        </div>
    </div>
    ...
    <div id="detailSheet" class="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#050b14] z-[140] rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.3)] transform translate-y-full transition-transform duration-300 max-w-md mx-auto border-t border-gray-200 dark:border-gray-800 pb-safe flex flex-col max-h-[85vh]">
        ...
        <div class="p-5 border-t border-gray-200 dark:border-gray-800 shrink-0 bg-white dark:bg-[#050b14]">
            ...
            <button id="btnLanjutkan" class="w-full py-3.5 bg-[#002147] dark:bg-yellow-400 text-white dark:text-[#001229] font-bold rounded-xl text-sm shadow-md hover:opacity-90 transition opacity-50 cursor-not-allowed" onclick="executeBuy()">Lanjutkan Pembayaran</button>
        </div>
    </div>
                ...
                if (provider.items && provider.items.length > 0 && type === 'data') {
                    ...
                    const html = provider.items.map(item => `
                        <div class="flex items-center px-5 py-4 border-b border-gray-200 dark:border-gray-800 cursor-pointer hover:bg-gray-50 dark:hover:bg-[#1a2639] transition" onclick="selectCategory('${item}', '${provider.placeholder}')">
                            <div class="w-8 h-8 rounded-full border border-black dark:border-gray-400 flex items-center justify-center text-[8px] shrink-0 bg-white dark:bg-[#0b1320]">${provider.isIcon ? `<i class="${provider.logo}"></i>` : provider.logo}</div>
...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
    <div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden transition-colors">
        ...
        <div class="fixed bottom-0 w-full max-w-md bg-white dark:bg-[#001229] border-t border-gray-200 dark:border-gray-800 flex justify-around p-3 pb-4 shadow-sm z-40 transition-colors">
            <div class="flex flex-col items-center cursor-pointer text-gray-400 hover:text-[#002147] dark:hover:text-yellow-400 transition" onclick="location.href='/dashboard.html'">
                <i class="fas fa-home text-xl"></i><span class="text-[10px] mt-1 font-bold">HOME</span>
            </div>
            <div class="flex flex-col items-center cursor-pointer text-gray-400 hover:text-[#002147] dark:hover:text-yellow-400 transition" onclick="location.href='/riwayat.html'">
                <i class="fas fa-file-alt text-xl"></i><span class="text-[10px] mt-1 font-bold">RIWAYAT</span>
    ...
    <script>
        ...
        fetch('/api/info').then(r => r.json()).then(data => { 
            ...
            if(data.info.length === 0) {
                list.innerHTML = `<div class="mt-20 flex flex-col items-center justify-center text-gray-400 dark:text-gray-500"><i class="fas fa-bell-slash text-5xl mb-4 opacity-50"></i><p class="text-sm">Belum ada info terbaru.</p></div>`; 
            } else {
                list.innerHTML = data.info.reverse().map(i => `
                    <div class="relative bg-white dark:bg-[#111c2e] border border-gray-200 dark:border-gray-800 rounded-2xl p-5 mb-4 shadow-sm overflow-hidden transition-colors">
                        ...
                        <div class="flex justify-between items-start mb-3 relative z-10">
                            ...
                            <span class="text-[10px] text-gray-500 whitespace-nowrap bg-gray-100 dark:bg-black px-2 py-1 rounded-md border border-gray-200 dark:border-gray-800">${i.date}</span>
                        </div>
...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
    <div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden transition-colors">
        ...
        <div class="bg-white dark:bg-[#050b14] p-8 pb-10 flex flex-col items-center relative rounded-b-[2rem] shadow-sm dark:shadow-lg transition-colors">
            <div class="w-24 h-24 bg-gray-100 dark:bg-[#1e293b] rounded-full flex justify-center items-center text-[#002147] dark:text-white font-extrabold text-4xl mt-2 mb-3 shadow-md dark:shadow-xl overflow-hidden border-2 border-transparent" id="profileCircle">U</div>
            <div class="flex items-center gap-3">
        ...
        <div class="mt-4 px-2">
            ...
            <div class="flex items-center px-4 py-5 border-b border-gray-200 dark:border-gray-800 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800/50" onclick="toggleChangePassword()">
                ...
                <i class="fas fa-chevron-down text-gray-400 text-sm transition-transform duration-300" id="cpwIcon"></i>
            </div>
            
            <div id="changePasswordBox" class="hidden px-6 py-5 border-b border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-[#070e18] slide-down">
                <div id="step1Cp">
                    <p class="text-xs text-gray-600 dark:text-gray-400 mb-3 text-center">Klik tombol di bawah untuk menerima kode OTP ke WA Anda.</p>
                ...
                <div id="step2Cp" class="hidden">
                    ...
                    <div class="compact-input-wrapper mb-5">
                        <input type="password" id="cpNewPassword" class="w-full bg-white dark:bg-[#0b1320] border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-gray-800 dark:text-white text-sm focus:outline-none focus:border-[#002147] dark:focus:border-yellow-400 text-center" placeholder="Ketik password baru">
                        <i class="fas fa-eye password-toggle" onclick="togglePassword('cpNewPassword', this)"></i>
                    </div>
    ...
    <div id="editProfileModal" class="fixed inset-0 z-[110] hidden flex items-center justify-center bg-black/70">
        <div class="bg-white dark:bg-[#0b1320] w-[90%] max-w-[340px] rounded-[1.25rem] border border-gray-200 dark:border-gray-800 shadow-2xl relative p-6 animate-slide-up">
            ...
            <div class="relative w-20 h-20 mx-auto mb-8">
                <div class="w-full h-full rounded-full border-2 border-[#002147] dark:border-yellow-400 flex items-center justify-center text-[#002147] dark:text-white text-3xl font-bold bg-gray-100 dark:bg-[#1e293b] overflow-hidden" id="editModalInitial">U</div>
            ...
            <div class="mb-6 hidden slide-down" id="editOtpContainer">
                <label class="block text-[10px] text-gray-500 dark:text-gray-400 mb-1 text-center">OTP dikirim ke WA baru</label>
                <input type="number" id="editOtpInput" class="w-full bg-white dark:bg-[#0b1320] border border-green-500 dark:border-green-700 rounded-lg px-3 py-3 text-gray-800 dark:text-white text-lg tracking-[0.5em] text-center font-bold focus:outline-none focus:border-green-400 transition-colors" placeholder="XXXX">
            </div>

            <button id="btnSimpanProfil" onclick="saveProfile()" class="w-full py-3.5 bg-[#002147] dark:bg-yellow-400 text-white dark:text-[#001229] font-bold rounded-xl mb-3 shadow-md transition">Simpan Profil</button>
    ...
    <script>
        ...
        function logout() {
            Swal.fire({ title: 'Keluar Akun?', text: "Apakah Anda yakin ingin keluar?", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#002147', confirmButtonText: 'Ya, Keluar', cancelButtonText: 'Batal', background: localStorage.getItem('darkMode') === 'true' ? '#0b1320' : '#ffffff', color: localStorage.getItem('darkMode') === 'true' ? '#fff' : '#000' })
            .then((result) => { if (result.isConfirmed) { localStorage.removeItem('user'); window.location.href = '/'; } });
        }
        ...
        function closeEditModal() {
            ...
            document.getElementById('btnSimpanProfil').classList.remove('bg-green-500', 'dark:bg-green-400');
            document.getElementById('editOtpInput').value = '';
        }

        async function saveProfile() {
            const oldPhone = user.phone; const newName = document.getElementById('editName').value; const rawPhone = document.getElementById('editPhone').value; 
            const newPhone = rawPhone.startsWith('0') ? '62' + rawPhone.slice(1) : rawPhone; const otp = document.getElementById('editOtpInput').value;
            ...
            if(newPhone !== oldPhone && !isRequestingOtp) {
                ...
                try {
                    ...
                    if(res.ok) {
                        isRequestingOtp = true; document.getElementById('editOtpContainer').classList.remove('hidden'); document.getElementById('btnSimpanProfil').innerText = 'Verifikasi OTP & Simpan'; document.getElementById('btnSimpanProfil').classList.add('bg-green-500', 'dark:bg-green-400');
        ...
        function deleteAccount() {
            Swal.fire({ title: 'Hapus Akun Permanen?', text: "Akun dan sisa saldo Anda akan hangus!", icon: 'error', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#002147', confirmButtonText: 'Ya, Hapus!', background: localStorage.getItem('darkMode') === 'true' ? '#0b1320' : '#ffffff', color: localStorage.getItem('darkMode') === 'true' ? '#fff' : '#000' }).then(async (result) => {
...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
    <div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden">
        ...
        <div class="mx-4 mt-4 bg-white dark:bg-[#111c2e] p-4 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm transition-colors">
    ...
    <script>
        ...
        fetch('/api/user/transactions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ phone: user.phone }) }).then(res => res.json()).then(data => { 
            const container = document.getElementById('historyContainer'); 
            if(data.transactions.length === 0) { 
                container.innerHTML = `<div class="mt-10 flex flex-col items-center justify-center text-center px-6"><div class="w-[5.5rem] h-[5.5rem] bg-gray-100 dark:bg-[#111c2e] rounded-full flex items-center justify-center mb-6 shadow-sm border border-gray-200 dark:border-gray-800 transition-colors"><i class="fas fa-receipt text-gray-400 text-4xl"></i></div><h2 class="text-gray-800 dark:text-white font-bold text-lg tracking-wide mb-2 transition-colors">Belum Ada Transaksi</h2><p class="text-gray-500 dark:text-gray-400 text-[13px] leading-relaxed mb-8 px-2 transition-colors">Ayo mulai transaksi pertamamu sekarang dan nikmati berbagai promo menarik!</p><button class="bg-[#002147] text-white dark:bg-[#0b1320] dark:text-yellow-400 border border-transparent dark:border-gray-700 font-bold py-3 px-8 rounded-full shadow-lg hover:opacity-90 transition" onclick="location.href='/dashboard.html'">Transaksi Sekarang</button></div>`; 
            } else { 
                window.trxData = data.transactions.reverse(); 
                container.innerHTML = window.trxData.map((item, index) => { 
                    let statusColor = item.status === 'Proses' ? 'text-yellow-600 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-900/30 border-yellow-200 dark:border-yellow-800' : (item.status === 'Sukses' ? 'text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 border-green-200 dark:border-green-800' : 'text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/30 border-red-200 dark:border-red-800'); 
                    return `<div onclick="showDetailTrx(${index})" class="bg-white dark:bg-[#111c2e] p-4 rounded-2xl mb-3 border border-gray-200 dark:border-gray-800 shadow-sm flex justify-between items-center transition-colors cursor-pointer hover:shadow-md hover:border-[#002147] dark:hover:border-yellow-400"><div class="flex items-center gap-3"><div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-[#0b1320] flex items-center justify-center text-gray-500 dark:text-gray-400 text-lg"><i class="fas fa-box"></i></div><div><h4 class="font-bold text-[13px] text-gray-800 dark:text-gray-200">${item.produk}</h4><p class="text-[10px] text-gray-500">${item.date}</p></div></div><div class="text-right"><p class="font-extrabold text-[14px] text-[#002147] dark:text-yellow-400 mb-1">Rp ${item.harga.toLocaleString('id-ID')}</p><span class="text-[9px] font-bold px-2 py-0.5 rounded border ${statusColor} uppercase tracking-wider">${item.status}</span></div></div>`; 
                }).join(''); 
            } 
        }).catch(err => { document.getElementById('historyContainer').innerHTML = `<div class="mt-20 text-center text-red-500">Gagal memuat data.</div>`; }); 

        window.showDetailTrx = function(index) { 
            const item = window.trxData[index]; 
            const isDark = localStorage.getItem('darkMode') === 'true'; 
            const bgPopup = isDark ? '#0b1320' : '#ffffff'; 
        ...
        <div class="px-4 mt-6">
            ...
            <div class="bg-white dark:bg-[#111c2e] rounded-b-2xl rounded-t-xl overflow-hidden border border-gray-200 dark:border-gray-800 shadow-sm transition-colors mt-4">
                ...
                <div class="p-4 grid grid-cols-3 gap-3">
                    ...
                    <div class="bg-gray-50 dark:bg-[#1a2639] border border-gray-200 dark:border-gray-700 rounded-[1rem] p-3 flex flex-col items-center justify-center cursor-pointer hover:border-[#002147] dark:hover:border-yellow-400 transition-colors h-28" onclick="location.href='/operator.html?type=game&provider=free+fire'">
                    ...
                    <div class="bg-gray-50 dark:bg-[#1a2639] border border-gray-200 dark:border-gray-700 rounded-[1rem] p-3 flex flex-col items-center justify-center cursor-pointer hover:border-[#002147] dark:hover:border-yellow-400 transition-colors h-28" onclick="location.href='/operator.html?type=game&provider=mobile+legends'">
                    ...
                    <div class="bg-gray-50 dark:bg-[#1a2639] border border-gray-200 dark:border-gray-700 rounded-[1rem] p-3 flex flex-col items-center justify-center cursor-pointer hover:border-[#002147] dark:hover:border-yellow-400 transition-colors h-28" onclick="location.href='/operator.html?type=game&provider=pubg+mobile'">
...
# ==========================================
# FILE NODE.JS (API + TELEGRAM COMMAND CENTER)
# ==========================================
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}          💎 PANEL DIGITAL FIKY STORE (V113) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
    ...
    case $choice in
        ...
        22)
            ...
            if [[ "$confirm_csv" == "y" || "$confirm_csv" == "Y" ]]; then
                cd "$HOME/$DIR_NAME"
                node -e "const fs=require('fs'); const file='./import.csv'; if(!fs.existsSync(file)){console.log('❌ File import.csv tidak ditemukan!'); process.exit();} const lines=fs.readFileSync(file, 'utf-8').split('\n'); let data=fs.existsSync('./local_products.json') ? JSON.parse(fs.readFileSync('./local_products.json')) : []; let count=0; lines.forEach(line=>{ let p=line.split(';'); if(p.length>=6){ data.push({ id:'LOC'+Date.now()+Math.floor(Math.random()*1000),
