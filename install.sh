...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
    ...
    <script>
        ...
        // State management: 'operator' -> 'category' -> 'product'
        let currentState = 'operator';
        let originalTitle = '';
        let currentProvider = null; // Menyimpan data provider yg sedang dipilih
        ...
        const ewalletCategories = {
            'dana': { name: 'DANA', logo: 'DN', placeholder: 'Masukkan Nomor DANA...', items: ['Top Up Saldo DANA'] },
            'gopay': { name: 'GOPAY', logo: 'GP', placeholder: 'Masukkan Nomor GoPay...', items: ['Top Up Saldo GOPAY'] },
            'shopeepay': { name: 'SHOPEEPAY', logo: 'SP', placeholder: 'Masukkan Nomor ShopeePay...', items: ['Top Up Saldo SHOPEEPAY'] },
            'ovo': { name: 'OVO', logo: 'OV', placeholder: 'Masukkan Nomor OVO...', items: ['Top Up Saldo OVO'] },
            'linkaja': { name: 'LINKAJA', logo: 'LA', placeholder: 'Masukkan Nomor LinkAja...', items: ['Top Up Saldo LINKAJA'] }
        };

        const etollCategories = {
            'bni': { name: 'BNI TapCash', logo: 'BNI', placeholder: 'Masukkan Nomor BNI TapCash...', items: ['Isi Saldo BNI TapCash'] },
            'bri': { name: 'BRI Brizzi', logo: 'BRI', placeholder: 'Masukkan Nomor BRI Brizzi...', items: ['Isi Saldo BRI Brizzi'] },
            'mandiri': { name: 'Mandiri Emoney', logo: 'MDR', placeholder: 'Masukkan Nomor Mandiri E-Money...', items: ['Isi Saldo Mandiri E-Money'] }
        };

        const plnCategories = {
            'pln_token': { name: 'Token PLN', logo: 'fas fa-bolt', isIcon: true, placeholder: 'Masukkan No Meter / ID Pelanggan PLN...', items: ['Beli Token PLN'] },
        ...
        tagihanCategories.forEach(t => {
            allProviders[t.id] = { name: t.name.replace('<br>', ' '), logo: t.icon, isIcon: true, placeholder: t.placeholder, items: ['Cek Tagihan'] };
        });
        ...
        // TAHAP 1: Milih Operator memunculkan Kategori (tanpa input nomor)
        function selectProvider(op) {
            ...
            if(provider) {
                ...
                if (provider.items && provider.items.length > 0) {
                    ...
                    document.getElementById('categoryList').innerHTML = `<div class="p-5 text-center text-gray-500">Tidak ada kategori</div>`;
                }
            } else {
        ...
        // TAHAP 2: Milih Kategori memunculkan Input Nomor & List Kosong
        function selectCategory(catName, placeholderStr) {
            ...
            // BLANK STATE UNTUK KATALOG PRODUK
            document.getElementById('productList').innerHTML = `
                <div class="py-16 flex flex-col items-center justify-center text-gray-400 dark:text-gray-600 transition-colors">
                    ...
                    <p class="text-[9px] mt-1">Produk akan ditambahkan di sini nantinya.</p>
                </div>
            `;
        }
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}           💎 PANEL DIGITAL FIKY STORE (V82) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
