...
echo "=========================================================="
echo "   MENGINSTAL DIGITAL FIKY STORE V115 (CLEAN & UNCUT)     "
echo "=========================================================="
...
.btn-yellow { width: 100%; padding: 0.625rem 1rem; background-color: #fde047; color: #002147; font-weight: bold; border-radius: 0.5rem; cursor: pointer; border: none; margin-top: 0.5rem; }
    ...
    document.getElementById('loginForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (res.ok) { 
          ...
          document.getElementById('loginDesc').innerHTML = "Kode OTP dikirim ke WA Anda.";
          Swal.close();
  ...
  <script>
    ...
    document.getElementById('otpForm').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const data = await res.json();
        if (res.ok) { 
          Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Akun aktif.', background: '#002147', color: '#fff' }).then(() => { window.location.href = '/'; }); 
        } else { Swal.fire({ icon: 'error', title: 'OTP Salah', text: data.error, background: '#002147', color: '#fff' }); }
      } catch (err) { Swal.fire({ icon: 'error', title: 'Oops...', text: 'Gagal verifikasi.', background: '#002147', color: '#fff' }); }
    });
  </script>
...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
  <div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden">
        ...
        <div class="flex-1 overflow-y-auto py-2">
          <ul class="text-[14px]">
            ...
            <li class="px-6 py-4 border-b border-gray-100 dark:border-[#0a2342] flex items-center justify-between cursor-pointer" onclick="toggleDarkMode()">
              ...
              <div class="w-11 h-[22px] bg-gray-300 rounded-full relative" id="darkModeToggleBg"><div class="w-[18px] h-[18px] bg-white rounded-full absolute top-[2px] left-[2px] shadow-md" id="darkModeToggleDot"></div></div>
            </li>
          </ul>
        </div>
        <div class="p-6">
    ...
    <div class="fixed bottom-0 w-full max-w-md bg-white dark:bg-[#001229] border-t border-gray-200 dark:border-gray-800 flex justify-around p-3 pb-4 shadow-2xl z-40">
      ...
      <div class="flex flex-col items-center cursor-pointer text-gray-400" onclick="location.href='/info.html'">
    ...
    <div id="topupSheet" class="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#050b14] z-[120] rounded-t-[2rem] transform translate-y-full transition-transform max-w-md mx-auto pb-safe">
      ...
      <div class="px-6 pb-6">
        ...
        <div class="flex items-center gap-3 mb-4">
          <input type="number" id="inputNominal" class="w-full bg-gray-50 dark:bg-[#0b1320] border border-gray-200 dark:border-gray-700 rounded-xl py-3 px-4 text-gray-800 dark:text-white font-bold" placeholder="Ketik nominal...">
        ...
        <div class="flex flex-col gap-3">
          <div onclick="selM('wa')" id="m-wa" class="flex items-center justify-between bg-gray-50 dark:bg-[#0b1320] border border-gray-200 dark:border-gray-800 p-3 rounded-xl cursor-pointer">
            <div class="flex items-center gap-3"><i class="fab fa-whatsapp text-2xl text-green-500"></i><div class="flex flex-col"><span class="font-bold text-sm dark:text-white">Manual WA</span><span class="text-[10px] text-gray-500 leading-tight">Transfer ke admin</span></div></div><div id="r-wa" class="w-5 h-5 rounded-full border-[3px] border-gray-400"></div>
          </div>
          <div onclick="selM('qris')" id="m-qris" class="flex items-center justify-between bg-gray-50 dark:bg-[#0b1320] border border-gray-200 dark:border-gray-800 p-3 rounded-xl cursor-pointer">
            <div class="flex items-center gap-3"><i class="fas fa-qrcode text-2xl dark:text-white"></i><div class="flex flex-col"><span class="font-bold text-sm dark:text-white">Otomatis QRIS</span><span class="text-[10px] text-gray-500 leading-tight">Otomatis masuk (Wajib 2 digit)</span></div></div><div id="r-qris" class="w-5 h-5 rounded-full border-[3px] border-gray-400"></div>
          </div>
        </div>
        <button onclick="location.href='/riwayat_topup.html'" class="w-full mt-4 py-3 bg-yellow-400 font-extrabold rounded-xl">Riwayat Top Up</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function updSal(){
      ...
      if(hideS){ el.innerText='Rp •••••••'; e.className='fas fa-eye-slash cursor-pointer text-gray-400'; }
      else{ el.innerText='Rp '+curSal.toLocaleString('id-ID'); e.className='fas fa-eye cursor-pointer text-gray-400'; }
    }
    ...
    function selM(m){
      ...
      document.getElementById('r-'+m).className='w-5 h-5 rounded-full border-[6px] border-yellow-400 bg-white';
    ...
    async function prosesTopup(){
      ...
      if(!n||n<=0) return Swal.fire({icon:'warning',title:'Gagal',text:'Isi nominal!',background:bg,color:c});
      ...
      if(sel === 'qris'){
        if(n<10000) return Swal.fire({icon:'warning',title:'Gagal',text:'Min 10rb',background:bg,color:c});
        closeTopUp();
        ...
        setTimeout(()=>{
          Swal.fire({
            ...
            html:`<p class="text-4xl text-yellow-500 font-extrabold mb-2">Rp ${fn.toLocaleString('id-ID')}</p><div class="bg-white p-3 rounded-xl inline-block border border-gray-200 mb-2"><img src="${qrisImg}" class="w-48 h-48 object-cover"></div><p class="text-xs text-blue-500">Transfer TEPAT sesuai nominal (3 digit akhir).</p>`,
        ...
        fetch('/api/topup/request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:user.phone,method:'Manual WA',nominal:fn})}).then(()=>{
          window.open(`https://wa.me/6282231154407?text=Topup%20Rp%20${fn.toLocaleString('id-ID')}`,'_blank');
...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
  <div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative shadow-2xl overflow-x-hidden flex flex-col">
    ...
    <div class="flex items-center p-5 bg-white dark:bg-[#0b1320] sticky top-0 z-40 border-b border-gray-200 dark:border-gray-800 shrink-0">
      <i class="fas fa-arrow-left text-xl cursor-pointer mr-4 dark:text-white" onclick="goBack()"></i>
      <h1 class="text-[18px] font-bold dark:text-white uppercase" id="pageTitle">Operator</h1>
    </div>
      ...
      <div id="productContainer" class="hidden">
        ...
        <div class="px-4 py-4 bg-white dark:bg-[#0b1320] border-b border-gray-200 dark:border-gray-800">
          ...
          <div class="relative flex items-center">
            <input type="text" id="inputTarget" class="w-full bg-gray-50 dark:bg-[#1a2639] border border-gray-300 dark:border-gray-700 dark:text-white rounded-xl py-3 pl-4 pr-24 text-sm font-bold focus:outline-none focus:border-yellow-400" placeholder="Ketik target...">
  ...
  <div id="detailSheet" class="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#050b14] z-[140] rounded-t-[2rem] transform translate-y-full transition-transform max-w-md mx-auto flex flex-col max-h-[85vh]">
    ...
    <div class="px-5 pb-2 border-b border-gray-200 dark:border-gray-800 shrink-0 flex justify-between">
      <h3 class="font-extrabold dark:text-white">Detail Produk</h3>
    ...
    <div class="p-5 overflow-y-auto hide-scrollbar flex-1">
      <div class="flex items-start gap-3 mb-4">
        ...
        <div>
          <h4 class="font-extrabold text-[15px] dark:text-white" id="dtName">-</h4>
          <p class="font-black text-lg text-yellow-500 mt-1" id="dtPrice">Rp 0</p>
        </div>
      </div>
    ...
    <div class="p-5 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-[#050b14]">
      <div class="flex gap-3 mb-3">
        <button class="flex-1 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 font-bold text-sm dark:text-white" onclick="closeDetail()">Kembali</button>
        <button class="flex-1 py-2.5 rounded-xl bg-green-600 text-white font-bold text-sm" onclick="bantuanAdmin()"><i class="fab fa-whatsapp mr-1"></i> Tanya CS</button>
      </div>
      <button id="btnLanjutkan" class="w-full py-3.5 bg-yellow-400 text-[#001229] font-bold rounded-xl text-sm opacity-50 cursor-not-allowed" onclick="executeBuy()">Lanjutkan Pembayaran</button>
    </div>
  </div>
  ...
  <script>
    ...
    for(let k in cL){
      let v = cL[k];
      h += `<div class="flex items-center p-4 border-b border-gray-200 dark:border-gray-800 cursor-pointer hover:bg-gray-100 dark:hover:bg-[#1a2639]" onclick="selectProvider('${k}')">
      ...
      try {
        ...
        if(d.data && d.data.length > 0) {
          l.innerHTML = d.data.map(p => {
            ...
            let bdg = p.is_open ? 
              `<span class="text-[8px] bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400 px-1.5 py-0.5 rounded font-bold uppercase">Tersedia</span>` : 
              `<span class="text-[8px] bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400 px-1.5 py-0.5 rounded font-bold uppercase">Gangguan</span>`;
            
            let clk = p.is_open ? `showProductDetail('${sS}','${sN}',${p.price},${p.isLocal},'${sD}')` : `Swal.fire({icon:'error',title:'Gangguan',text:'Produk Gangguan Server.',background:'#0b1320',color:'#fff'})`;
    ...
    function selectProvider(o) {
      ...
      if(pr) {
        ...
        if(pr.items && pr.items.length > 0 && t === 'data') {
          ...
          let h = '';
          pr.items.forEach(i => {
            h += `<div class="flex items-center px-5 py-4 border-b border-gray-200 dark:border-gray-800 cursor-pointer hover:bg-gray-100 dark:hover:bg-[#1a2639]" onclick="selectCategory('${i}')"><div class="flex-1 text-[13px] font-medium text-gray-800 dark:text-gray-200 uppercase">${i}</div></div>`;
          });
...
# ==========================================
# FILE HTML LAINNYA (PROFILE, RIWAYAT, INFO DLL)
# ==========================================
cat << 'EOF' > public/profile.html
<!DOCTYPE html><html lang="id" id="html-root"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Profil</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><link rel="stylesheet" href="style.css"><script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script><script>tailwind.config={darkMode:'class'}</script></head><body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300"><div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden"><div class="bg-white dark:bg-[#050b14] p-8 pb-10 flex flex-col items-center relative rounded-b-[2rem] shadow-sm dark:shadow-lg"><div class="w-24 h-24 bg-gray-100 dark:bg-[#1e293b] rounded-full flex justify-center items-center text-[#002147] dark:text-white font-extrabold text-4xl mt-2 mb-3 shadow-md" id="profileCircle">U</div><div class="flex items-center gap-3"><h2 class="text-2xl font-bold tracking-wide text-gray-800 dark:text-gray-100" id="profileName">User</h2></div></div><div class="mt-4 px-2"><div class="flex items-center px-4 py-5 border-b border-gray-200 dark:border-gray-800"><i class="fas fa-phone-alt text-[#002147] dark:text-gray-400 w-10 text-xl text-center"></i><div class="flex-1 text-[15px] font-bold text-gray-800 dark:text-gray-200 ml-2">No. Telp</div><div class="text-sm text-gray-500" id="profilePhoneData">08...</div></div><div class="flex items-center px-4 py-5 border-b border-gray-200 dark:border-gray-800"><i class="fas fa-wallet text-[#002147] dark:text-gray-400 w-10 text-xl text-center"></i><div class="flex-1 text-[15px] font-bold text-gray-800 dark:text-gray-200 ml-2">Saldo Akun</div><div class="text-sm font-bold text-[#002147] dark:text-yellow-400" id="profileSaldo">Rp 0</div></div><div class="flex items-center px-4 py-5 cursor-pointer hover:bg-red-50 dark:hover:bg-gray-800/50 transition" onclick="logout()"><i class="fas fa-sign-out-alt text-red-600 w-10 text-xl text-center"></i><div class="flex-1 text-[15px] font-bold text-red-600 ml-2">Keluar Akun</div></div></div><div class="fixed bottom-0 w-full max-w-md bg-white dark:bg-[#001229] border-t border-gray-200 dark:border-gray-800 flex justify-around p-3 pb-4 shadow-sm z-40"><div class="flex flex-col items-center cursor-pointer text-gray-400" onclick="location.href='/dashboard.html'"><i class="fas fa-home text-xl"></i><span class="text-[10px] mt-1 font-bold">HOME</span></div><div class="flex flex-col items-center cursor-pointer text-[#002147] dark:text-yellow-400"><i class="fas fa-user text-xl"></i><span class="text-[10px] mt-1 font-bold">PROFIL</span></div></div></div><script>const user=JSON.parse(localStorage.getItem('user'));if(!user)window.location.href='/';if(localStorage.getItem('darkMode')==='true'||localStorage.getItem('darkMode')===null)document.getElementById('html-root').classList.add('dark');document.getElementById('profileName').innerText=user.name;document.getElementById('profilePhoneData').innerText=user.phone;fetch('/api/user/balance',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:user.phone})}).then(r=>r.json()).then(d=>{document.getElementById('profileSaldo').innerText='Rp '+d.saldo.toLocaleString('id-ID');});function logout(){Swal.fire({title:'Keluar?',icon:'warning',showCancelButton:true,background:'#0b1320',color:'#fff'}).then(r=>{if(r.isConfirmed){localStorage.removeItem('user');window.location.href='/';}});}</script></body></html>
EOF

cat << 'EOF' > public/riwayat.html
<!DOCTYPE html><html lang="id" id="html-root"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Riwayat Transaksi</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script><script>tailwind.config={darkMode:'class'}</script></head><body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300"><div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden"><div class="flex items-center p-5 bg-white dark:bg-[#0b1320] sticky top-0 z-40 border-b border-gray-200 dark:border-gray-800"><i class="fas fa-arrow-left text-xl cursor-pointer mr-4 dark:text-white" onclick="location.href='/dashboard.html'"></i><h1 class="text-[18px] font-bold dark:text-white">Riwayat Transaksi</h1></div><div class="px-4 mt-4" id="historyContainer"><div class="mt-14 flex flex-col items-center justify-center text-center px-6"><i class="fas fa-spinner fa-spin text-4xl mb-4 text-gray-400"></i></div></div></div><script>const user=JSON.parse(localStorage.getItem('user'));if(!user)window.location.href='/';if(localStorage.getItem('darkMode')===null)document.getElementById('html-root').classList.add('dark');fetch('/api/user/transactions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:user.phone})}).then(r=>r.json()).then(d=>{const c=document.getElementById('historyContainer');if(d.transactions.length===0){c.innerHTML=`<div class="mt-10 text-center"><h2 class="dark:text-white font-bold">Belum Ada Transaksi</h2></div>`;}else{window.trxData=d.transactions.reverse();c.innerHTML=window.trxData.map((i,idx)=>{let sc=i.status==='Proses'?'text-yellow-500 border-yellow-800':(i.status==='Sukses'?'text-green-500 border-green-800':'text-red-500 border-red-800');return `<div onclick="showDetailTrx(${idx})" class="bg-white dark:bg-[#111c2e] p-4 rounded-2xl mb-3 border border-gray-800 shadow-sm flex justify-between cursor-pointer"><div><h4 class="font-bold text-[13px] dark:text-gray-200">${i.produk}</h4><p class="text-[10px] text-gray-500">${i.date}</p></div><div class="text-right"><p class="font-extrabold text-[14px] text-yellow-400 mb-1">Rp ${i.harga.toLocaleString('id-ID')}</p><span class="text-[9px] font-bold px-2 py-0.5 rounded border ${sc} uppercase">${i.status}</span></div></div>`;}).join('');}});window.showDetailTrx=function(idx){const i=window.trxData[idx];Swal.fire({title:'Detail',html:`Produk: ${i.produk}<br>Tujuan: ${i.no_tujuan}<br>SN: ${i.sn_ref||'-'}`,background:'#0b1320',color:'#fff'});}</script></body></html>
EOF

cat << 'EOF' > public/riwayat_topup.html
<!DOCTYPE html><html lang="id" id="html-root"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Riwayat Top Up</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><script>tailwind.config={darkMode:'class'}</script></head><body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300"><div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden"><div class="flex items-center p-5 bg-white dark:bg-[#0b1320] sticky top-0 z-40 border-b border-gray-200 dark:border-gray-800"><i class="fas fa-arrow-left text-xl cursor-pointer mr-4 dark:text-white" onclick="location.href='/dashboard.html'"></i><h1 class="text-[18px] font-bold dark:text-white">Riwayat Topup</h1></div><div class="px-4 mt-4" id="historyContainer"><div class="mt-14 flex flex-col items-center justify-center text-center px-6"><i class="fas fa-spinner fa-spin text-4xl mb-4 text-gray-400"></i></div></div></div><script>const user=JSON.parse(localStorage.getItem('user'));if(!user)window.location.href='/';if(localStorage.getItem('darkMode')===null)document.getElementById('html-root').classList.add('dark');fetch('/api/topup/history',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:user.phone})}).then(r=>r.json()).then(d=>{const c=document.getElementById('historyContainer');if(d.history.length===0){c.innerHTML=`<div class="mt-10 text-center"><h2 class="dark:text-white font-bold">Belum Ada Topup</h2></div>`;}else{window.topupData=d.history.reverse();c.innerHTML=window.topupData.map((i,idx)=>{let sc=i.status==='Proses'?'text-yellow-500 border-yellow-800':(i.status==='Sukses'?'text-green-500 border-green-800':'text-red-500 border-red-800');return `<div class="bg-white dark:bg-[#111c2e] p-4 rounded-2xl mb-3 border border-gray-800 shadow-sm flex justify-between items-center"><div><h4 class="font-bold text-[13px] dark:text-gray-200">${i.method}</h4><p class="text-[10px] text-gray-500">${i.date}</p></div><div class="text-right"><p class="font-extrabold text-[14px] text-yellow-400 mb-1">Rp ${i.nominal.toLocaleString('id-ID')}</p><span class="text-[9px] font-bold px-2 py-0.5 rounded border ${sc} uppercase">${i.status}</span></div></div>`;}).join('');}});</script></body></html>
EOF

cat << 'EOF' > public/info.html
<!DOCTYPE html><html lang="id" id="html-root"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Info</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><script>tailwind.config={darkMode:'class'}</script></head><body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300"><div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden"><div class="flex items-center p-5 bg-white dark:bg-[#0b1320] sticky top-0 z-40 border-b border-gray-800"><i class="fas fa-arrow-left text-xl cursor-pointer mr-4 dark:text-white" onclick="location.href='/dashboard.html'"></i><h1 class="text-[18px] font-bold dark:text-white">Pusat Informasi</h1></div><div class="p-4" id="infoList"><div class="mt-20 text-center text-gray-400">Memuat...</div></div></div><script>const user=JSON.parse(localStorage.getItem('user'));if(!user)window.location.href='/';if(localStorage.getItem('darkMode')===null)document.getElementById('html-root').classList.add('dark');fetch('/api/info').then(r=>r.json()).then(d=>{const l=document.getElementById('infoList');if(d.info.length===0)l.innerHTML='<div class="text-center mt-10">Kosong</div>';else l.innerHTML=d.info.reverse().map(i=>`<div class="bg-white dark:bg-[#111c2e] border border-gray-800 rounded-2xl p-5 mb-4"><h3 class="font-bold text-yellow-400 mb-2">${i.judul}</h3><p class="text-sm dark:text-gray-300">${i.isi}</p><p class="text-[10px] text-gray-500 mt-2">${i.date}</p></div>`).join('');});</script></body></html>
EOF

cat << 'EOF' > public/mutasi.html
<!DOCTYPE html><html lang="id" id="html-root"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Mutasi</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><script>tailwind.config={darkMode:'class'}</script></head><body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300"><div class="max-w-md mx-auto bg-gray-50 dark:bg-[#0b1320] min-h-screen relative pb-24 shadow-2xl overflow-x-hidden"><div class="flex items-center p-5 bg-white dark:bg-[#0b1320] sticky top-0 z-40 border-b border-gray-800"><i class="fas fa-arrow-left text-xl cursor-pointer mr-4 dark:text-white" onclick="location.href='/dashboard.html'"></i><h1 class="text-[18px] font-bold dark:text-white">Mutasi Saldo</h1></div><div class="p-4" id="mutasiList"><div class="mt-20 text-center text-gray-400">Memuat...</div></div></div><script>const user=JSON.parse(localStorage.getItem('user'));if(!user)window.location.href='/';if(localStorage.getItem('darkMode')===null)document.getElementById('html-root').classList.add('dark');fetch('/api/user/mutasi',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:user.phone})}).then(r=>r.json()).then(d=>{const l=document.getElementById('mutasiList');if(d.mutasi.length===0)l.innerHTML='<div class="text-center mt-10">Kosong</div>';else l.innerHTML=d.mutasi.reverse().map(m=>`<div class="bg-white dark:bg-[#111c2e] border border-gray-800 rounded-2xl p-4 mb-3 flex justify-between"><div><h4 class="font-bold text-[13px] dark:text-gray-200">${m.desc}</h4><p class="text-[10px] text-gray-500">${m.date}</p></div><div class="font-bold text-[14px] ${m.type==='in'?'text-green-500':'text-red-500'}">${m.type==='in'?'+':'-'} Rp ${m.amount.toLocaleString('id-ID')}</div></div>`).join('');});</script></body></html>
EOF

# ==========================================
# FILE NODE.JS (API + TELEGRAM + CSV)
# RAPI, JELAS, DAN TIDAK DIKOMPRES
# ==========================================
    ...
    if (!isLocal && config.digiUser && config.digiKey) {
      try {
        ...
        if (digiData.status === 'Gagal') {
          db[phone].saldo += price; saveJSON(dbFile, db); return res.status(400).json({ error: digiData.message || 'Gagal dari provider.' });
...
app.get('/api/admin/backup', async (req, res) => {
  let config = loadJSON(configFile);
  if(!config.teleToken || !config.teleChatId) return res.status(400).json({ error: "Token/Chat ID Telegram belum disetting di Panel." });
  try {
...
// FIX OTP ERROR V115 (Mencegah Spasi Terselip saat Login/Register)
...
app.post('/api/auth/register', async (req, res) => {
  ...
  const otp = Math.floor(1000 + Math.random() * 9000).toString(); 
  webUsers[fPhone] = { name, email, password, isVerified: false, otp, otpExpiry: Date.now() + 300000, avatar: null }; saveJSON(webUsersFile, webUsers);
...
app.post('/api/auth/forgot', async (req, res) => {
  ...
  try { await global.waSocket?.sendMessage(fPhone + '@c.us', { text: `Kode OTP Reset Password:\n*${otp}*` }); res.json({ message: 'OTP Terkirim' }); } catch(e) { res.status(500).json({ error: 'Gagal kirim WA.' }); }
});
...
while true; do clear
  echo -e "${CYAN}======================================================${NC}"
  echo -e "${YELLOW}          💎 PANEL DIGITAL FIKY STORE (V115) 💎       ${NC}"
  echo -e "${CYAN}======================================================${NC}"
  echo ""
  ...
  case $choice in
    ...
    20)
      ...
      read -p "Enter..." ;;
    21) 
      echo -e "${RED}Pastikan Anda sudah mem-push script ke GitHub Anda!${NC}"
...
pm2 restart all > /dev/null 2>&1
echo "  SISTEM WEB V115 BERHASIL DIPERBARUI SECARA PENUH!       "
