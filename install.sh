#!/bin/bash
# ==========================================================
# DIGITAL FIKY STORE - V165.1 (UPDATE RESI QRIS & NOTIFIKASI)
# ==========================================================
...
echo "=========================================================="
echo "    MENGINSTAL DIGITAL FIKY STORE V165.1 (PART 1)         "
echo "=========================================================="
...
<body class="bg-[#0b1320] font-sans transition-colors duration-300 text-white">
    ...
    <div id="topupSheet" class="fixed bottom-0 left-0 right-0 bg-[#111c2e] z-[120] rounded-t-[2rem] transform translate-y-full transition-transform max-w-md mx-auto pb-safe border-t border-[#1e293b]">
      ...
      <div class="px-6 pb-6">
        ...
        <div class="flex justify-between gap-2 mb-6">
          <button onclick="document.getElementById('inputNominal').value=10000" class="flex-1 bg-[#0b1320] border border-[#1e293b] text-white font-extrabold py-2.5 rounded-xl hover:border-[#facc15]">10K</button>
          <button onclick="document.getElementById('inputNominal').value=20000" class="flex-1 bg-[#0b1320] border border-[#1e293b] text-white font-extrabold py-2.5 rounded-xl hover:border-[#facc15]">20K</button>
          <button onclick="document.getElementById('inputNominal').value=50000" class="flex-1 bg-[#0b1320] border border-[#1e293b] text-white font-extrabold py-2.5 rounded-xl hover:border-[#facc15]">50K</button>
          <button onclick="document.getElementById('inputNominal').value=100000" class="flex-1 bg-[#0b1320] border border-[#1e293b] text-white font-extrabold py-2.5 rounded-xl hover:border-[#facc15]">100K</button>
        </div>
  ...
  <script>
    ...
    async function prosesTopup() {
      ...
      const fn = n + Math.floor(Math.random() * 90) + 10; // KODE UNIK
      ...
      if(sel === 'qris') {
        ...
        Swal.fire({ title: 'Membuat QRIS Dinamis...', allowOutsideClick: false, background: bg, color: c, didOpen: () => Swal.showLoading() });

        try {
            // MENGIRIM BASE NOMINAL DAN NOMINAL TOTAL AGAR BACKEND TAHU KODE UNIKNYA
            let res = await fetch('/api/topup/request', { 
                method: 'POST', 
                headers: {'Content-Type': 'application/json'}, 
                body: JSON.stringify({phone: user.phone, method: 'QRIS Otomatis', nominal: fn, base_nominal: n}) 
            });
        ...
        closeTopUp();
        fetch('/api/topup/request', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({phone: user.phone, method: 'Manual WA', nominal: fn, base_nominal: n}) }).then(() => {
...
<body class="bg-[#0b1320] font-sans transition-colors duration-300 text-white min-h-screen flex justify-center">
  <div class="w-full max-w-md bg-[#0b1320] relative shadow-2xl flex flex-col h-full min-h-screen pb-10">
    ...
    <div class="px-4 flex-1">
      <div class="bg-[#111c2e] rounded-[1.5rem] border border-[#1e293b] p-5 shadow-lg flex flex-col items-center">
        ...
        <div class="bg-white p-4 rounded-3xl inline-block mb-6 w-full max-w-[280px] shadow-sm flex justify-center relative">
            <img id="qrisImage" src="" class="w-64 h-64 object-contain" style="image-rendering: crisp-edges;">
        </div>

        <!-- TOMBOL UNDUH, BAGIKAN, CEK RIWAYAT -->
        <div class="flex w-full gap-3 justify-center mb-6">
            <button onclick="downloadQR()" class="flex-1 bg-[#1a2639] border border-gray-600 text-gray-200 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:border-[#facc15] transition-colors shadow-sm text-xs">
                <i class="fas fa-download"></i> Unduh
            </button>
            <button onclick="shareQR()" class="flex-1 bg-[#1a2639] border border-gray-600 text-gray-200 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:border-[#facc15] transition-colors shadow-sm text-xs">
                <i class="fas fa-share-alt"></i> Bagikan
            </button>
            <button onclick="window.location.href='/riwayat_topup.html'" class="flex-1 bg-[#facc15] text-[#0b1320] py-2.5 rounded-xl font-extrabold flex items-center justify-center gap-2 hover:opacity-90 transition-colors shadow-sm text-xs">
                <i class="fas fa-history"></i> Riwayat
            </button>
        </div>

        <!-- TAMPILAN RESI MEWAH -->
        <div class="bg-[#0b1320] border border-[#1e293b] rounded-xl p-4 mb-2 w-full text-left text-gray-300 text-[12px] font-mono leading-relaxed shadow-inner overflow-hidden relative">
            <div class="absolute top-0 left-0 w-1 h-full bg-[#facc15]"></div>
            
            <div class="text-[#facc15] font-bold text-center mb-3 text-[13px]">⏳ TOP UP MENUNGGU PEMBAYARAN ⏳</div>
            
            <div class="grid grid-cols-[20px_auto] gap-x-1 gap-y-1 mb-3">
                <span>👤</span><span>Nama: <b class="text-white" id="detName"></b></span>
                <span>✉️</span><span>Email: <b class="text-white" id="detEmail"></b></span>
                <span>📱</span><span>WA: <b class="text-white" id="detPhone"></b></span>
                <span>⌚️</span><span>Waktu: <b class="text-white" id="detWaktu"></b></span>
                <span>🏦</span><span>Metode: <b class="text-white">QRIS Otomatis</b></span>
            </div>

            <div class="grid grid-cols-[20px_auto] gap-x-1 gap-y-1 mb-3">
                <span>💰</span><span>Jumlah Deposit: Rp <b class="text-white" id="detBase"></b></span>
                <span>🎫</span><span>Kode Unik: <b class="text-[#facc15]" id="detUnik"></b></span>
                <span>💵</span><span>Total Saldo Diterima: Rp <b class="text-green-400" id="detTotal"></b></span>
            </div>

            <div class="mb-1 font-bold text-white mt-4"><span class="mr-1">💳</span>Riwayat Saldo</div>
            <div class="grid grid-cols-[20px_auto] gap-x-1 gap-y-1 pl-1">
                <span>📉</span><span>Saldo Sebelum: Rp <b class="text-white" id="detSalSeb"></b></span>
                <span>📈</span><span>Saldo Sesudah: Rp <b class="text-white" id="detSalSes"></b> <span class="text-[#facc15]">(Pending)</span></span>
            </div>
        </div>

      </div>
      
      <button onclick="window.location.href='/dashboard.html'" class="w-full py-4 mt-6 bg-[#ef4444] text-white font-extrabold rounded-[1rem] shadow-lg hover:bg-red-600 transition-colors text-[15px]">
          Kembali ke Beranda
      </button>
  ...
  <script>
    ...
    // MENGISI DATA KE RESI MEWAH
    document.getElementById('detName').innerText = user.name;
    document.getElementById('detEmail').innerText = user.email || 'Tidak ada email';
    document.getElementById('detPhone').innerText = user.phone;
    document.getElementById('detWaktu').innerText = qData.date;
    document.getElementById('detBase').innerText = (qData.base_nominal || 0).toLocaleString('id-ID');
    document.getElementById('detUnik').innerText = qData.kode_unik || 0;
    document.getElementById('detTotal').innerText = qData.nominal.toLocaleString('id-ID');
    document.getElementById('detSalSeb').innerText = (qData.saldo_sebelum || 0).toLocaleString('id-ID');
    document.getElementById('detSalSes').innerText = (qData.saldo_sebelum || 0).toLocaleString('id-ID');
    ...
    const checkInterval = setInterval(() => {
        ...
        .then(d => {
            ...
            // Jika ada mutasi masuk dengan nominal sama
        ...
        try {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    ...
                    text: `Tolong bayarkan tagihan Top Up sebesar Rp ${qData.nominal.toLocaleString('id-ID')}`,
                    files: [file]
                });
            } else {
...
echo "[PART 1 SELESAI BOSKUUU!]"
echo "[PART 2 DITULIS: Menyusun Dashboard, Operator, Game, Riwayat Topup, Info, dan Mutasi...]"
...
<body class="bg-[#0b1320] font-sans transition-colors duration-300 text-white">
  ...
  <script>
    ...
    function showProductDetail(sku, n, pr, il, ds, isOpen) {
      sS = sku; sN = n; sP = pr; sL = il; isProductOpen = isOpen;
    ...
    function executeBuy() {
      ...
      if(isMaintenance()) {
          return Swal.fire({
              icon: 'error', title: 'MAINTENANCE', text: 'Sistem sedang Maintenance Otomatis (23:00 - 00:30 WIB). Transaksi ditutup sementara.', 
...
<body class="bg-[#0b1320] font-sans transition-colors duration-300 text-white">
  ...
  <script>
            ...
            c.innerHTML = filtered.map((i) => {
                ...
                <div onclick="showDetailTopup(${rawIdx})" class="bg-[#111c2e] p-4 rounded-[1.2rem] mb-3 border border-[#1e293b] shadow-sm cursor-pointer hover:bg-[#1a2639] transition-colors flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex flex-col"><h4 class="font-extrabold text-[14px] text-gray-200 mb-1">Topup ${methodClean}</h4><span class="text-[11px] text-gray-400 font-medium">${i.date}</span></div>
                    </div>
    ...
    window.komplainTopup = function(nominal, tanggal, statusLengkap) {
        let msg = `Halo, saya ingin komplain deposit dengan nominal Rp ${nominal} pada tanggal ${tanggal}. Status: ${statusLengkap}.`;
        window.open(`https://wa.me/6282231154407?text=` + encodeURIComponent(msg), '_blank');
    }

    // 🔥 FORMAT RESI MEWAH UPDATE 🔥
    window.showDetailTopup = function(index) {
        const item = window.topupData[index];
        let rawStatus = item.status.toLowerCase();
        let isExp = rawStatus === 'expired';
        
        if(rawStatus === 'proses' && item.method.includes('QRIS')) rawStatus = 'pending_qris';
        if(isExp) rawStatus = 'gagal_kedaluwarsa';
        
        let methodClean = item.method.includes('QRIS') ? 'QRIS Otomatis' : (item.method.includes('Admin') ? 'Admin Fiky Store' : 'Manual WA');

        let isSuccess = item.status === 'Sukses';
        let isPending = item.status === 'Proses';
        
        let headerIcon = isSuccess ? '✅' : (isPending ? '⏳' : '❌');
        let headerText = isSuccess ? 'TOP UP BERHASIL' : (isPending ? 'MENUNGGU PEMBAYARAN' : 'TOP UP GAGAL');
        let headerColor = isSuccess ? 'text-green-500' : (isPending ? 'text-[#facc15]' : 'text-red-500');
        let sideColor = isSuccess ? 'bg-green-500' : (isPending ? 'bg-[#facc15]' : 'bg-red-500');

        let baseNom = item.base_nominal || item.nominal;
        let unik = item.kode_unik || (item.nominal - baseNom);
        let salSeb = item.saldo_sebelum || 0;
        let salSes = isSuccess ? (salSeb + item.nominal) : salSeb;
        let salSesText = isSuccess ? `Rp ${salSes.toLocaleString('id-ID')}` : `Rp ${salSes.toLocaleString('id-ID')} <span class="text-[#facc15]">(Pending)</span>`;

        let htmlContent = `
        <div class="bg-[#0b1320] border border-[#1e293b] rounded-xl p-4 w-full text-left text-gray-300 text-[12px] font-mono leading-relaxed shadow-inner overflow-hidden relative mt-2 mb-4">
            <div class="absolute top-0 left-0 w-1 h-full ${sideColor}"></div>

            <div class="${headerColor} font-bold text-center mb-3 text-[13px]">${headerIcon} ${headerText} ${headerIcon}</div>

            <div class="grid grid-cols-[20px_auto] gap-x-1 gap-y-1 mb-3">
                <span>👤</span><span>Nama: <b class="text-white">${user.name}</b></span>
                <span>✉️</span><span>Email: <b class="text-white">${user.email || '-'}</b></span>
                <span>📱</span><span>WA: <b class="text-white">${user.phone}</b></span>
                <span>⌚️</span><span>Waktu: <b class="text-white">${item.date}</b></span>
                <span>🏦</span><span>Metode: <b class="text-white">${methodClean}</b></span>
            </div>

            <div class="grid grid-cols-[20px_auto] gap-x-1 gap-y-1 mb-3">
                <span>💰</span><span>Jumlah Deposit: Rp <b class="text-white">${baseNom.toLocaleString('id-ID')}</b></span>
                <span>🎫</span><span>Kode Unik: <b class="text-[#facc15]">${unik}</b></span>
                <span>💵</span><span>Total Saldo Diterima: Rp <b class="text-green-400">${(item.nominal || 0).toLocaleString('id-ID')}</b></span>
            </div>

            <div class="mb-1 font-bold text-white mt-4"><span class="mr-1">💳</span>Riwayat Saldo</div>
            <div class="grid grid-cols-[20px_auto] gap-x-1 gap-y-1 pl-1">
                <span>📉</span><span>Saldo Sebelum: Rp <b class="text-white">${salSeb.toLocaleString('id-ID')}</b></span>
                <span>📈</span><span>Saldo Sesudah: ${salSesText}</span>
            </div>
        </div>
        <button onclick="komplainTopup('${item.nominal}', '${item.date}', '${rawStatus}')" class="w-full py-3.5 bg-[#ef4444] hover:bg-[#dc2626] text-white font-extrabold rounded-xl mb-3 transition-colors text-[13px]">Hubungi Admin (Komplain)</button>
        <button onclick="Swal.close()" class="w-full py-3.5 bg-transparent border border-[#334155] text-white hover:bg-[#1a2639] font-extrabold rounded-xl transition-colors text-[13px]">Tutup Resi</button>
        `;
...
echo "Selesai Part 2"
echo "[PART 4 DITULIS: Menyusun Profil, Riwayat Utama, Backend Server & Notifikasi Mewah...]"
      ...
      let htmlContent = `
        ...
        <div class="bg-[#0b1320] border border-[#1e293b] rounded-xl p-4 mb-5 text-left">
            ...
            <div class="flex justify-between mb-3">
                ...
                <span class="${i.status === 'Sukses' ? 'text-green-400' : (i.status === 'Proses' ? 'text-[#facc15]' : 'text-red-400')} font-extrabold text-[13px] text-right uppercase">${i.status}</span>
            </div>
            <div class="flex justify-between mb-3">
...
app.get('/api/config', (req, res) => { 
    let cfg = loadJSON(configFile);
    res.json({ banners: cfg.banners || [], linkTele: cfg.linkTele || 'https://t.me/digitalfikystore_channel', linkWa: cfg.linkWa || 'https://whatsapp.com/channel/digitalfikystore' }); 
});
...
app.get('/api/global-stats', (req, res) => {
    ...
    const tzStr = new Date().toLocaleString("en-US", {timeZone: "Asia/Jakarta"}); let now = new Date(tzStr);
...
app.post('/api/transaction/create', async (req, res) => {
    try {
        ...
        let db = loadJSON(dbFile); let config = loadJSON(configFile); let webUsers = loadJSON(webUsersFile); let uData = webUsers[phone] || { name: 'Unknown', email: '-' };
        ...
        if (!isLocal && config.digiUser && config.digiKey) {
            ...
            } catch(e) { db[phone].saldo += price; saveJSON(dbFile, db); return res.status(400).json({ error: 'Koneksi ke Digiflazz Timeout. Saldo dikembalikan.' }); }
        }
...
setInterval(async () => {
    ...
    for (let phone in db) {
        ...
        let uData = webUsers[phone] || { name: 'Unknown', email: '-' };
...
function convertCRC16(str) {
    let crc = 0xFFFF;
    for (let c = 0; c < str.length; c++) { crc ^= str.charCodeAt(c) << 8; for (let i = 0; i < 8; i++) { if (crc & 0x8000) crc = (crc << 1) ^ 0x1021; else crc = crc << 1; } }
    let hex = (crc & 0xFFFF).toString(16).toUpperCase(); return hex.padStart(4, '0');
}

function generateDynamicQris(staticQris, amount) {
    try { let qrisWithoutCRC = staticQris.substring(0, staticQris.indexOf("6304")); if(!qrisWithoutCRC) return staticQris; let amountStr = amount.toString(); let amountLen = amountStr.length.toString().padStart(2, '0'); let tag54 = "54" + amountLen + amountStr; let rawQris = qrisWithoutCRC + tag54 + "6304"; let newCrc = convertCRC16(rawQris); return rawQris + newCrc; } catch (e) { return staticQris; }
}

// 💰 FUNGSI AUTO-QRIS CHECKER (BHM PROTECTED) 💰
setInterval(async () => {
    let db = loadJSON(dbFile); let config = loadJSON(configFile); let webUsers = loadJSON(webUsersFile); let changed = false;
    ...
    for (let phone in db) {
        if (db[phone].topup) {
            db[phone].topup.forEach(t => { if (t.status === 'Proses' && t.method === 'QRIS Otomatis') { pendingQris.push({ phone, topup: t }); } });
        }
    }
    ...
    if (pendingQris.length > 0) {
        try {
            ...
            let res = await axios.get(endpoint, { headers: { 'Authorization': `Bearer ${config.bhmToken}` }, timeout: 10000 }).catch(async () => { return await axios.get('http://gopay.bhm.biz.id/api/transactions', { headers: { 'Authorization': `Bearer ${config.bhmToken}` }, timeout: 10000 }); });
            
            let rawData = res.data.items || res.data.data || res.data;
            ...
            for (let p of pendingQris) {
                ...
                let matchTx = txs.find(tx => {
                    let amount = parseInt(parseFloat(tx.amount));
                    let isCredit = (tx.type && tx.type.toLowerCase() === 'credit') || (tx.status && tx.status.toLowerCase() === 'settlement') || parseFloat(tx.amount) > 0;
                    return amount === targetNominal && isCredit;
                });

                if (matchTx) {
                    let txId = matchTx.transaction_id || matchTx.external_id || matchTx.id || Date.now().toString();
                    if (!config.processedGopay) config.processedGopay = [];
                    if (!config.processedGopay.includes(txId)) {
                        config.processedGopay.push(txId);
                        if(config.processedGopay.length > 500) config.processedGopay.shift();
                        saveJSON(configFile, config);

                        let baseNom = p.topup.base_nominal || targetNominal;
                        let unik = targetNominal - baseNom;
                        let salSeb = p.topup.saldo_sebelum !== undefined ? p.topup.saldo_sebelum : db[p.phone].saldo;
                        let salSes = db[p.phone].saldo + targetNominal;
                        let dateSukses = new Date().toLocaleString('id-ID', {timeZone: 'Asia/Jakarta'});

                        p.topup.status = 'Sukses';
                        db[p.phone].saldo += targetNominal;
                        db[p.phone].mutasi.push({ id: 'TU' + Date.now(), type: 'in', amount: targetNominal, desc: 'Topup QRIS Dinamis (GoPay)', date: dateSukses });
                        changed = true;

                        let uData = webUsers[p.phone] || {name: 'Unknown', email: '-'};
                        let msgSuccess = `✅ *TOP UP BERHASIL* ✅\n\n👤 Nama: ${uData.name}\n✉️ Email: ${uData.email || '-'}\n📱 WA: ${p.phone}\n⌚️ Waktu: ${dateSukses}\n🏦 Metode: QRIS Otomatis\n\n💰 Jumlah Deposit: Rp ${baseNom.toLocaleString('id-ID')}\n🎫 Kode Unik: ${unik}\n💵 Total Saldo Diterima: Rp ${targetNominal.toLocaleString('id-ID')}\n\n💳 Riwayat Saldo\n📉 Saldo Sebelum: Rp ${salSeb.toLocaleString('id-ID')}\n📈 Saldo Sesudah: Rp ${salSes.toLocaleString('id-ID')}\n\n🔖 Ref GoPay: ${txId}`;
                        
                        sendTeleNotif(msgSuccess, 'topup');
                        if (global.waSocket) global.waSocket.sendMessage(p.phone + '@s.whatsapp.net', {text: msgSuccess}).catch(e=>{});
                    }
                }
            }
        } catch (e) {}
...
// ==========================================
// API TOPUP LOGIC (RESI MEWAH PENDING)
// ==========================================
app.post('/api/topup/request', (req, res) => {
    const { phone, method, nominal, base_nominal } = req.body; 
    let config = loadJSON(configFile);
    if (isMaintenance()) return res.status(400).json({ error: 'Sistem sedang Maintenance Otomatis.' });
    
    let db = loadJSON(dbFile); let webUsers = loadJSON(webUsersFile); let uData = webUsers[phone] || { name: 'Unknown', email: '-' };
    if (!db[phone]) db[phone] = { saldo: 0, jid: phone + '@s.whatsapp.net', mutasi: [], topup: [], transactions: [] };
    if (!db[phone].topup) db[phone].topup = [];
    
    let expiry = null; let finalQrisString = null;
    let baseNom = base_nominal || nominal;
    let unik = nominal - baseNom;

    if (method === 'QRIS Otomatis') {
        expiry = Date.now() + 10*60*1000;
        if (config.qrisStringCode && config.qrisStringCode !== '') { finalQrisString = generateDynamicQris(config.qrisStringCode, nominal); } 
        else { return res.status(400).json({ error: 'Admin belum menyetting String QRIS di panel VPS. Hubungi Admin.' }); }
    }

    let dateStr = new Date().toLocaleString('id-ID', {timeZone: 'Asia/Jakarta'});
    let newRef = 'TP-' + Date.now();
    let salSeb = db[phone].saldo || 0;

    const newTopup = { id: newRef, method: method, nominal: nominal, base_nominal: baseNom, saldo_sebelum: salSeb, status: 'Proses', date: dateStr, expiry: expiry };
    db[phone].topup.push(newTopup); saveJSON(dbFile, db); 
    
    let methodClean = method === 'QRIS Otomatis' ? 'QRIS Otomatis' : 'Manual WA';
    let msgTopup = `⏳ *TOP UP MENUNGGU PEMBAYARAN* ⏳\n\n👤 Nama: ${uData.name}\n✉️ Email: ${uData.email || '-'}\n📱 WA: ${phone}\n⌚️ Waktu: ${dateStr}\n🏦 Metode: ${methodClean}\n\n💰 Jumlah Deposit: Rp ${baseNom.toLocaleString('id-ID')}\n🎫 Kode Unik: ${unik}\n💵 Total Saldo Diterima: Rp ${nominal.toLocaleString('id-ID')}\n\n💳 Riwayat Saldo\n📉 Saldo Sebelum: Rp ${salSeb.toLocaleString('id-ID')}\n📈 Saldo Sesudah: Rp ${salSeb.toLocaleString('id-ID')} (Pending)`;
    
    sendTeleNotif(msgTopup, 'topup'); 
    if (global.waSocket) global.waSocket.sendMessage(phone + '@s.whatsapp.net', { text: msgTopup }).catch(e=>{});
...
app.post('/api/admin/balance', async (req, res) => {
    ...
    let uData = webUsers[targetPhone] || {name: 'Unknown', email: '-'};
    let salSeb = db[targetPhone].saldo || 0;
    let salSes = salSeb + parseInt(amount);
    ...
    if (action === 'add') { 
        ...
        db[targetPhone].topup.push({ id: 'TU'+Date.now(), method: 'Admin Fiky Store', nominal: parseInt(amount), base_nominal: parseInt(amount), saldo_sebelum: salSeb, status: 'Sukses', date: dateStr }); 
        saveJSON(dbFile, db); 
        
        let msgSuccess = `✅ *TOP UP BERHASIL* ✅\n\n👤 Nama: ${uData.name}\n✉️ Email: ${uData.email || '-'}\n📱 WA: ${targetPhone}\n⌚️ Waktu: ${dateStr}\n🏦 Metode: Admin Fiky Store\n\n💰 Jumlah Deposit: Rp ${parseInt(amount).toLocaleString('id-ID')}\n🎫 Kode Unik: 0\n💵 Total Saldo Diterima: Rp ${parseInt(amount).toLocaleString('id-ID')}\n\n💳 Riwayat Saldo\n📉 Saldo Sebelum: Rp ${salSeb.toLocaleString('id-ID')}\n📈 Saldo Sesudah: Rp ${salSes.toLocaleString('id-ID')}\n\n🔖 Ref: Admin`;
        
        sendTeleNotif(msgSuccess, 'topup'); 
        if (global.waSocket) global.waSocket.sendMessage(targetPhone + '@s.whatsapp.net', { text: msgSuccess }).catch(e=>{});
        ...
        saveJSON(dbFile, db); res.json({ success: true, message: `\n✅ Saldo berhasil dikurangi!` }); 
    }
});
...
app.post('/api/auth/forgot', async (req, res) => {
    ...
    try { await global.waSocket?.sendMessage(fPhone + '@s.whatsapp.net', { text: `Permintaan reset password *DIGITAL FIKY STORE*.\n\nKode OTP:\n\n*${otp}*\n\n⏳ _Berlaku 5 menit._` }); res.json({ message: 'OTP Terkirim' }); } 
...
app.post('/api/auth/request-update-otp', async (req, res) => {
    ...
    try { await global.waSocket?.sendMessage((fNew !== fOld ? fNew : fOld) + '@s.whatsapp.net', { text: `Kode OTP verifikasi ubah keamanan akun *DIGITAL FIKY STORE*:\n\n*${otp}*\n\n⏳ _Berlaku 5 menit._` }); res.json({ message: 'OTP Terkirim' }); } 
    catch(e) { res.status(500).json({ error: 'Gagal kirim WA.' }); } 
});
...
app.get('*', (req, res) => {
    ...
    if (!reqPath.includes('.') && reqPath !== '/') { let altPath = path.join(__dirname, 'public', reqPath + '.html'); if (fs.existsSync(altPath)) return res.sendFile(altPath); }
    if (reqPath.endsWith('.html')) { let pagePath = path.join(__dirname, 'public', reqPath); if (fs.existsSync(pagePath)) return res.sendFile(pagePath); }
    res.status(404).send(`<div style="background-color:#0b1320; color:white; font-family:sans-serif; text-align:center; padding-top:50px; height:100vh;"><h1 style="color:#facc15;">FILE TIDAK DITEMUKAN (404)</h1><p>Halaman <b>${reqPath}</b> tidak ada di server.</p><br><button onclick="window.location.href='/dashboard.html'" style="background-color:#facc15; border:none; padding:10px 20px; font-weight:bold; border-radius:5px; cursor:pointer; color:#0b1320;">KEMBALI KE DASHBOARD</button></div>`); 
});
...
while true; do 
    ...
    SALDO_DIGI=$(node -e "
        ...
        if(!cfg.digiUser || !cfg.digiKey) { console.log('Belum Disetting'); } else {
...
const fs = require('fs'); const file = './config.json'; let config = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {}; config.botNumber = process.argv[2]; fs.writeFileSync(file, JSON.stringify(config, null, 2)); console.log('Nomor berhasil disimpan!');
JS
        ...
        6)
            ...
            echo -e "${CYAN}===============================================${NC}"
            read -p "Judul Informasi : " b_judul
            read -p "Isi Pengumuman  : " b_isi
            if [ ! -z "$b_isi" ]; then
                cd "$HOME/$DIR_NAME"
                cat << 'JS' > temp_broadcast.js
const http = require('http'); const data = JSON.stringify({ judul: process.argv[2], message: process.argv[3] });
const req = http.request({ hostname: 'localhost', port: 3000, path: '/api/admin/broadcast', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(data) } }, res => { console.log('Terkirim ke Server!'); }); req.write(data); req.end();
JS
                node temp_broadcast.js "$b_judul" "$b_isi"
...
const fs = require('fs'); const db = fs.existsSync('./database.json') ? JSON.parse(fs.readFileSync('./database.json')) : {}; const users = fs.existsSync('./web_users.json') ? JSON.parse(fs.readFileSync('./web_users.json')) : {};
console.log('\n--- DAFTAR SALDO MEMBER ---');
for (let p in users) { if (users[p].isVerified) { console.log('- ' + users[p].name + ' (' + p + ') : Rp ' + (db[p] ? db[p].saldo : 0)); } } console.log('---------------------------\n');
JS
...
const http = require('http'); const data = JSON.stringify({ identifier: process.argv[2], amount: parseInt(process.argv[3]), action: 'add' });
const req = http.request({ hostname: 'localhost', port: 3000, path: '/api/admin/balance', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(data) } }, res => { res.on('data', c => console.log(c.toString())); }); req.write(data); req.end();
JS
...
const http = require('http'); const data = JSON.stringify({ identifier: process.argv[2], amount: parseInt(process.argv[3]), action: 'reduce' });
const req = http.request({ hostname: 'localhost', port: 3000, path: '/api/admin/balance', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(data) } }, res => { res.on('data', c => console.log(c.toString())); }); req.write(data); req.end();
JS
...
const fs = require('fs'); const path = './public/banners'; if(!fs.existsSync(path)) fs.mkdirSync(path, {recursive:true});
let rawFiles = fs.readdirSync(path).filter(f => f.match(/\.(jpg|jpeg|png|gif)$/i));
rawFiles.forEach(f => { if(f.includes(' ')) { let newName = f.replace(/ /g, '_'); fs.renameSync(path + '/' + f, path + '/' + newName); } });
let finalFiles = fs.readdirSync(path).filter(f => f.match(/\.(jpg|jpeg|png|gif)$/i));
let cfg = fs.existsSync('./config.json') ? JSON.parse(fs.readFileSync('./config.json')) : {}; cfg.banners = finalFiles; fs.writeFileSync('./config.json', JSON.stringify(cfg, null, 2)); console.log('✅ Berhasil menyinkronkan ' + finalFiles.length + ' banner! (Bug Spasi Fixed via Rename)');
JS
                node temp_banner.js
                rm temp_banner.js
                pm2 restart $BOT_NAME > /dev/null 2>&1
...
const fs = require('fs'); let cfg = fs.existsSync('./config.json') ? JSON.parse(fs.readFileSync('./config.json')) : {}; cfg.banners = []; fs.writeFileSync('./config.json', JSON.stringify(cfg, null, 2)); console.log('✅ Semua banner telah disembunyikan dari aplikasi.');
JS
...
const fs = require('fs'); const file = './config.json'; let cfg = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {};
cfg.markupRules = { m1: parseInt(process.argv[2]) || 0, m2: parseInt(process.argv[3]) || 0, m3: parseInt(process.argv[4]) || 0, m4: parseInt(process.argv[5]) || 0, m5: parseInt(process.argv[6]) || 0, m6: parseInt(process.argv[7]) || 0, m7: parseInt(process.argv[8]) || 0, m8: parseInt(process.argv[9]) || 0, m9: parseInt(process.argv[10]) || 0, m10: parseInt(process.argv[11]) || 0, m11: parseInt(process.argv[12]) || 0, m12: parseInt(process.argv[13]) || 0, m13: parseInt(process.argv[14]) || 0, m14: parseInt(process.argv[15]) || 0 };
fs.writeFileSync(file, JSON.stringify(cfg, null, 2)); console.log('\n✅ 14 Tingkatan Keuntungan (God Mode) Berhasil Disimpan!');
JS
...
const fs = require('fs'); let f = './local_products.json'; let data = fs.existsSync(f) ? JSON.parse(fs.readFileSync(f)) : [];
...
fs.writeFileSync(f, JSON.stringify(data, null, 2)); console.log('✅ Produk Berhasil Ditambahkan!');
JS
...
const fs = require('fs'); let f = './local_products.json'; let data = fs.existsSync(f) ? JSON.parse(fs.readFileSync(f)) : [];
if (data.length === 0) { console.log('\nBelum ada produk lokal.'); } else { console.log('\n--- DAFTAR PRODUK LOKAL ---'); data.forEach((p, i) => console.log('[' + i + '] ' + p.name + ' - Rp' + p.price)); }
JS
...
const fs = require('fs'); let f = './local_products.json'; let data = fs.existsSync(f) ? JSON.parse(fs.readFileSync(f)) : []; let idx = parseInt(process.argv[2]);
if (data[idx]) { data.splice(idx, 1); fs.writeFileSync(f, JSON.stringify(data, null, 2)); console.log('\n✅ Produk berhasil dihapus!'); } else { console.log('\n❌ Nomor ID tidak valid.'); }
JS
...
EOFNGINX
        ...
        13)
            ...
            cd "$HOME/$DIR_NAME"
            cat << 'JS' > temp_digi.js
const fs = require('fs'); let file = './config.json'; let cfg = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {}; cfg.digiUser = process.argv[2]; cfg.digiKey = process.argv[3]; fs.writeFileSync(file, JSON.stringify(cfg, null, 2)); console.log('✅ Konfigurasi API Digiflazz Disimpan!');
JS
        ...
        14)
            ...
            if [ ! -z "$bhm_token" ] && [ ! -z "$bhm_phone" ] && [ ! -z "$bhm_mid" ]; then
                ...
                if [ ! -z "$bhm_otp" ]; then
                    ...
                    ver_otp=$(curl -sS -X POST http://gopay.bhm.biz.id/v1/gopay/merchants/$bhm_mid/connect/verify-otp -H "Authorization: Bearer $bhm_token" -H 'Content-Type: application/json' -d "{\"otp\":\"$bhm_otp\"}")
            ...
            cd "$HOME/$DIR_NAME"
            cat << 'JS' > temp_bhm.js
const fs = require('fs'); let file = './config.json'; let cfg = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {};
...
const fs = require('fs'); let f = './digi_cache.json'; if(fs.existsSync(f)) { fs.unlinkSync(f); console.log('✅ Cache Katalog berhasil dihapus!'); } else { console.log('✅ Cache sudah bersih.'); }
JS
...
const fs = require('fs'); let file = './config.json'; let cfg = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {}; cfg.teleTokenTrx = process.argv[2]; cfg.teleChatIdTrx = process.argv[3]; fs.writeFileSync(file, JSON.stringify(cfg, null, 2));
JS
...
const fs = require('fs'); let file = './config.json'; let cfg = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {}; cfg.teleTokenTopup = process.argv[2]; cfg.teleChatIdTopup = process.argv[3]; fs.writeFileSync(file, JSON.stringify(cfg, null, 2));
JS
...
const fs = require('fs'); let file = './config.json'; let cfg = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {}; cfg.teleTokenBackup = process.argv[2]; cfg.teleChatIdBackup = process.argv[3]; fs.writeFileSync(file, JSON.stringify(cfg, null, 2));
JS
        ...
        17)
            ...
            cd "$HOME/$DIR_NAME"
            cat << 'JS' > temp_auto.js
const fs = require('fs'); let file = './config.json'; let cfg = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file)) : {}; cfg.autoBackupHours = parseFloat(process.argv[2]); fs.writeFileSync(file, JSON.stringify(cfg, null, 2)); console.log('✅ Disimpan: Auto Backup dijalankan setiap ' + process.argv[2] + ' Jam!');
JS
...
let cfg = fs.existsSync('./config.json') ? JSON.parse(fs.readFileSync('./config.json')) : {}; let t = cfg.teleTokenBackup || cfg.teleToken; let c = cfg.teleChatIdBackup || cfg.teleChatId;
if(!t || !c) { console.log('❌ Token/Chat ID Bot Backup belum disetting (Buka Menu 16)'); process.exit(); }
...
exec('zip -r ' + zipName + ' database.json web_users.json config.json local_products.json info.json', (err) => {
    const form = new FormData(); form.append('chat_id', c); form.append('document', fs.createReadStream(zipName)); form.append('caption', '📦 *BACKUP MANUAL BERHASIL*\n\nTanggal: ' + new Date().toLocaleString('id-ID', {timeZone: 'Asia/Jakarta'})); form.append('parse_mode', 'Markdown');
    axios.post('https://api.telegram.org/bot' + t + '/sendDocument', form, { headers: form.getHeaders() }).then(() => { console.log('✅ File Backup Berhasil Terkirim ke Telegram Anda!'); fs.unlinkSync(zipName); }).catch(e => { console.log('❌ Gagal Mengirim ke Telegram. Pastikan Token/Chat ID Benar!'); });
});
JS
...
echo -e "\033[1;33m     🚀 SEMUA UPDATE RESI MEWAH & NOTIFIKASI BERHASIL DITERAPKAN!   \033[0m"
echo -e "\033[0;32m======================================================================\033[0m"
echo -e "\033[0;36mFITUR BARU DI V165.1:\033[0m"
echo -e "  ✅ \033[1;33mRESI MEWAH WA & TELEGRAM\033[0m Format resi Top Up dipercantik!"
echo -e "  ✅ \033[1;33mRESI POPUP DI APLIKASI\033[0m Riwayat Top Up muncul popup Resi!"
echo -e "  ✅ \033[1;33mFIX KODE UNIK\033[0m Tampil Base Nominal, Kode Unik & Total Saldo."
echo -e "  ✅ \033[1;33mTOMBOL QRIS MEWAH\033[0m Tombol Download, Share, & Cek Riwayat."
echo -e "\033[0;32m======================================================================\033[0m"
