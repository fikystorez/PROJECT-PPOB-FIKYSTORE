...
// V104: BACKUP & RESTORE TELEGRAM API
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}          💎 PANEL DIGITAL FIKY STORE (V104) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
    ...
    echo -e "  ${GREEN}18.${NC} 💾 BACKUP DATA KE TELEGRAM"
...
EOFNGINX
        ...
        16) 
            ...
            echo -e "${CYAN}===============================================${NC}"
            echo -e "Menghapus file cache sistem dan menarik ulang data dari Digiflazz..."
            node -e "const fs=require('fs'); let f='$HOME/$DIR_NAME/digi_cache.json'; if(fs.existsSync(f)){ fs.unlinkSync(f); console.log('✅ Cache lokal dihapus!'); }"
        ...
        19)
            clear
            echo -e "${CYAN}===============================================${NC}"
            echo -e "${YELLOW}        📥 RESTORE DATA DARI DIRECT LINK       ${NC}"
            echo -e "${CYAN}===============================================${NC}"
            echo -e "Dapatkan direct link (link download langsung) dari file backup .zip Anda."
            echo ""
            read -p "Masukkan Direct Link ZIP: " link_restore
            if [ ! -z "$link_restore" ]; then
                cd "$HOME/$DIR_NAME"
                echo -e "${YELLOW}Mendownload file backup...${NC}"
                wget -qO restore.zip "$link_restore"
                if [ -f "restore.zip" ]; then
                    ...
                    echo -e "${RED}❌ Gagal mendownload file dari link tersebut! Pastikan linknya valid (Direct Link).${NC}"
                fi
            else
