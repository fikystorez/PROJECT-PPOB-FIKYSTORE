...
# ==========================================
# FILE NODE.JS (API + IRON CACHE SYSTEM)
# ==========================================
...
const digiCacheFile = './digi_cache.json'; // IRON CACHE V101
...
if (!fs.existsSync(digiCacheFile)) saveJSON(digiCacheFile, { time: 0, data: [] }); // IRON CACHE INIT
...
app.post('/api/products', async (req, res) => {
    ...
    let digiCache = loadJSON(digiCacheFile); // Panggil dari memori HDD
    let filtered = [];
    
    if(config.digiUser && config.digiKey) {
        // Cek jika sudah lewat 5 menit ATAU data kosong
        if (Date.now() - digiCache.time > 300000 || !digiCache.data || digiCache.data.length === 0) { 
            try {
                ...
                if(digiRes.data && digiRes.data.data) {
                    ...
                    saveJSON(digiCacheFile, digiCache); // Simpan permanen ke file
                }
            } catch(e) {
                console.log("[DIGIFLAZZ API] Gagal Fetch Katalog. Memakai data cache lama. Pesan:", e.message);
            }
        }
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}          💎 PANEL DIGITAL FIKY STORE (V101) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
...
EOFNGINX
        ...
        16) 
            ...
            ;;
        14) 
            echo -e "${RED}PERINGATAN: Jangan lakukan Update Sistem kecuali file di Github sudah diperbarui ke versi V101!${NC}"
