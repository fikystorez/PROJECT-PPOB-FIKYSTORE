...
# PART 1 DARI 4: SETUP BASH, CSS, & HALAMAN AUTHENTIKASI
# ==========================================================
...
echo "=========================================================="
echo "    MENGINSTAL DIGITAL FIKY STORE V166 (PART 1 DARI 4)    "
echo "=========================================================="
...
echo "[4/10] Membangun Halaman Autentikasi (FULL UNCOMPRESSED)..."
cd "$HOME/$DIR_NAME"
...
// selesai
echo "[5/10] Membangun Halaman Dashboard (PART 2 DARI 4 - FULL UNCOMPRESSED)..."
        ...
        <div id="sidebar" class="fixed inset-0 z-[100] transform -translate-x-full transition-transform duration-300 flex">
            ...
            <div class="absolute top-0 left-0 w-[80%] max-w-[300px] h-full bg-white dark:bg-[#0b1320] shadow-2xl flex flex-col border-r border-slate-200 dark:border-[#1e293b] transition-colors duration-300">
                <div class="p-8 pb-4 flex flex-col items-center relative border-b border-slate-200 dark:border-[#1e293b]">
                    ...
                    <div class="w-[4.5rem] h-[4.5rem] bg-slate-100 dark:bg-[#111c2e] rounded-full flex justify-center items-center text-orange-500 dark:text-[#facc15] text-4xl mb-3 shadow-md overflow-hidden border border-slate-200 dark:border-[#1e293b] transition-colors" id="sidebarInitial">
            ...
            if(sel === 'qris') {
                ...
                fetch('/api/topup/request', { 
                    ...
                    if(r.ok) {
                        ...
                        Swal.fire({ icon: 'error', title: 'Gagal', text: data.error, background: bg, color: c }); 
                    }
                });
            }
        }
...
// selesai
echo "[6/8] Membangun Halaman Game & Riwayat Topup (PART 3 DARI 4 - FULL UNCOMPRESSED)..."
...
<body class="bg-slate-50 dark:bg-[#0b1320] font-sans transition-colors duration-300 text-slate-800 dark:text-white">
    <div class="max-w-md mx-auto bg-slate-50 dark:bg-[#0b1320] min-h-screen relative shadow-2xl overflow-x-hidden transition-colors">
    ...
    <script>
        ...
        function filterHistory() {
            ...
            if(currentFilter !== 'Semua') { 
                filtered = currentFilter === 'Gagal' 
                    ? filtered.filter(i => i.status === 'Gagal' || i.status === 'Expired') 
                    : filtered.filter(i => i.status === currentFilter); 
            }
            ...
            if(!filtered || filtered.length === 0) {
                ...
                c.innerHTML = filtered.map((i) => {
                    ...
                    let dateClass = isDark ? 'text-gray-400' : 'text-slate-500';
        ...
        window.komplainTopup = function(nominal, tanggal, statusLengkap) {
            let msg = `Halo Admin, saya ingin komplain deposit dengan nominal ${nominal} pada tanggal ${tanggal}. Status saat ini: ${statusLengkap}. Mohon dibantu pengecekannya.`;
        ...
        window.showDetailTopup = function(index) {
            ...
            if(rawStatus === 'proses' && item.method.includes('QRIS')) {
                rawStatus = 'pending_qris';
            }
            if(rawStatus === 'expired') {
                rawStatus = 'gagal_kedaluwarsa';
            }
            ...
            // INJEKSI KODE QRIS DI BAGIAN ATAS SAAT STATUS PROSES
            let qrisBlock = '';
            if (rawStatus === 'pending_qris' && item.qris_string) {
                let qrisImg = `https://api.qrserver.com/v1/create-qr-code/?size=500x500&margin=2&data=${encodeURIComponent(item.qris_string)}`;
                qrisBlock = `
                <div class="mb-5 pb-5 border-b ${borderColor} text-center transition-colors">
                    <p class="text-[11px] font-extrabold ${labelColor} mb-2 uppercase tracking-widest transition-colors">Scan QRIS Untuk Membayar</p>
                    <div class="bg-white p-2 rounded-xl inline-block shadow-sm border border-slate-200 mb-3">
                        <img src="${qrisImg}" class="w-40 h-40 object-contain mx-auto" style="image-rendering: crisp-edges;">
                    </div>
                    <p class="text-[11px] font-black text-red-500 mb-1 bg-red-500/10 border border-red-500/30 py-1.5 px-3 rounded-lg inline-block w-full">Wajib persis Rp ${nominalLengkap.toLocaleString('id-ID')}</p>
                    <p class="text-[12px] font-bold text-slate-500 dark:text-gray-400 mt-2">Sisa Waktu: <span id="swal-timer-${index}" class="text-orange-500 dark:text-[#facc15] font-black text-[14px]">Menghitung...</span></p>
                </div>
                `;
            }
            ...
            <div class="${innerBg} border ${borderColor} rounded-xl p-4 mb-5 text-left overflow-y-auto max-h-[60vh] hide-scrollbar shadow-inner transition-colors">
                ...
                <div class="flex justify-between mb-5"><span class="${labelColor} font-black text-[13px] transition-colors">Total Diterima</span><span class="${highlightColor} font-black text-[15px] text-right transition-colors">Rp ${nominalLengkap.toLocaleString('id-ID')}</span></div>
            ...
            Swal.fire({ 
                ...
                padding: 0,
                didOpen: () => {
                    if (rawStatus === 'pending_qris' && item.expiry) {
                        const timerEl = document.getElementById(`swal-timer-${index}`);
                        if (timerEl) {
                            swalTimerInterval = setInterval(() => {
                                let now = Date.now();
                                let distance = item.expiry - now;
                                if (distance <= 0) {
                                    clearInterval(swalTimerInterval);
                                    timerEl.innerText = "KEDALUWARSA";
                                    timerEl.classList.replace("text-orange-500", "text-red-500");
                                    timerEl.classList.replace("dark:text-[#facc15]", "dark:text-red-500");
                                } else {
                                    let m = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
                                    let s = Math.floor((distance % (1000 * 60)) / 1000);
                                    timerEl.innerText = `${m} Menit ${s} Detik`;
                                }
                            }, 1000);
                        }
                    }
                ...
                willClose: () => {
                    if(swalTimerInterval) clearInterval(swalTimerInterval);
                }
            });
        }
    </script>
...
echo "[7/8] Membangun Halaman Info, Mutasi, Profile & Riwayat Produk (FULL UNCOMPRESSED)..."
cd "$HOME/$DIR_NAME"
...
// selesai
echo "[8/10] Menulis logika Backend Node.js (PART 4 DARI 4 - FULL UNCOMPRESSED)..."
...
function censorEmail(email) {
    ...
    if (namePart.length <= 3) {
        return namePart + 'xxx@' + domainPart;
    }
    return namePart.substring(0, 4) + 'xxx@' + domainPart;
}
...
function censorWa(wa) {
    ...
    return wa.substring(0, 4) + 'xxxx' + wa.substring(wa.length - 3);
}
...
const broadcastPublik = async (message, type, status) => {
    ...
    // Kirim ke Grup Telegram Publik (Mengirim semua status, tidak ada batasan 'Sukses' saja untuk Telegram)
    ...
    // Kirim ke Saluran WhatsApp Publik - HANYA SAAT STATUS SUKSES ATAU MAINTENANCE!
    if ((status === 'Sukses' || status === 'Maintenance') && cfg.waChannelId && global.waSocket) {
        try {
...
// ==========================================
// ALARM MAINTENANCE OTOMATIS (22:50 WIB)
// ==========================================
let lastMaintenanceNotifDate = '';
setInterval(() => {
    const tzStr = new Date().toLocaleString("en-US", {timeZone: "Asia/Jakarta"});
    const now = new Date(tzStr);
    const h = now.getHours();
    const m = now.getMinutes();
    const dateStr = now.getDate() + '-' + now.getMonth();
    
    if (h === 22 && m === 50 && lastMaintenanceNotifDate !== dateStr) {
        lastMaintenanceNotifDate = dateStr;
        
        const msg = `📢 *PENGUMUMAN PENTING* 📢\n\nSistem akan memasuki waktu *Maintenance Otomatis* dari pukul *23:00 WIB hingga 00:30 WIB*.\n\nSeluruh transaksi (Pembelian Produk & Top Up Saldo) akan dihentikan sementara. Mohon selesaikan transaksi Anda sebelum waktu tersebut.\n\nTerima kasih,\n*DIGITAL FIKY STORE*`;
        
        // 1. Masukkan ke Pusat Informasi Web
        let infoData = loadJSON(infoFile);
        infoData.push({ 
            judul: "🔧 MAINTENANCE SISTEM", 
            isi: "Sistem akan maintenance pukul 23:00 - 00:30 WIB. Transaksi ditutup sementara.", 
            date: new Date().toLocaleString('id-ID', {timeZone: 'Asia/Jakarta'}) 
        });
        if(infoData.length > 20) infoData = infoData.slice(-20);
        saveJSON(infoFile, infoData);
        
        // 2. Broadcast ke Telegram dan WA Channel
        broadcastPublik(msg, 'info', 'Maintenance');
    }
    ...
    for (let phone in db) {
        ...
        userTrx.forEach(trx => {
            if(trx.status === 'Sukses' || trx.status === 'Proses') {
                tAll++; 
                try {
                    let dParts = trx.date.split(',')[0].split('/');
                    if (dParts.length === 3) {
                        let trxDateObj = new Date(dParts[2], parseInt(dParts[1])-1, dParts[0]);
                        if (trxDateObj.getTime() === startOfDay.getTime()) tToday++;
                        if (trxDateObj >= startOfWeek) tWeek++;
                        if (trxDateObj >= startOfMonth) tMonth++;
                    }
                } catch(e) {}
            }
        });
...
app.post('/api/transaction/create', async (req, res) => {
    try {
        ...
        // LIMIT 50 TRANSAKSI (MENCEGAH DATABASE FULL)
        if(db[phone].mutasi.length > 50) db[phone].mutasi = db[phone].mutasi.slice(-50);
        if(db[phone].transactions.length > 50) db[phone].transactions = db[phone].transactions.slice(-50);
        ...
        // PENGIRIMAN NOTIF PUBLIK (DENGAN SENSOR)
            ...
            if (trx.status === 'Proses' && !trx.isLocal && trx.sku) {
                try {
                    ...
                    if (digiData.status === 'Sukses') { 
                        ...
                        user.mutasi.push({ id: 'REF'+Date.now(), type: 'in', amount: trx.harga, desc: `Refund: ${trx.produk}`, date: dateStr }); 
                        if(user.mutasi.length > 50) user.mutasi = user.mutasi.slice(-50);
...
function generateDynamicQris(staticQris, amount) {
    try {
        ...
        let amountStr = amount.toString(); let amountLen = amountStr.length.toString().padStart(2, '0');
        let tag54 = "54" + amountLen + amountStr; let rawQris = qrisWithoutCRC + tag54 + "6304";
        return rawQris + convertCRC16(rawQris);
    } catch (e) { return staticQris; }
}

// ==========================================
// 💰 FUNGSI AUTO-QRIS CHECKER (BHM BIZ)
// ==========================================
setInterval(async () => {
    ...
    if (pendingQris.length > 0) {
        try {
            let endpoint = config.bhmMerchantId ? `http://gopay.bhm.biz.id/v1/gopay/merchants/${config.bhmMerchantId}/transactions` : `http://gopay.bhm.biz.id/v1/gopay/transactions`;
            let res = await axios.get(endpoint, { headers: { 'Authorization': `Bearer ${config.bhmToken}` }, timeout: 10000 }).catch(async () => {
                return await axios.get('http://gopay.bhm.biz.id/api/transactions', { headers: { 'Authorization': `Bearer ${config.bhmToken}` }, timeout: 10000 });
            });
            
            let rawData = res.data; let txs = [];
            if (rawData && Array.isArray(rawData.items)) { txs = rawData.items; } else if (rawData && rawData.data && Array.isArray(rawData.data.items)) { txs = rawData.data.items; } else if (Array.isArray(rawData)) { txs = rawData; } else if (rawData && Array.isArray(rawData.data)) { txs = rawData.data; }
            ...
            for (let p of pendingQris) {
                ...
                let matchTx = txs.find(tx => {
                    let txAmountStr = String(tx.amount || tx.gross_amount || 0); let amount = parseInt(parseFloat(txAmountStr));
                ...
                if (matchTx) {
                    ...
                    if (refId && !config.processedGopay.includes(refId)) {
                        ...
                        let salSebelum = db[p.phone].saldo; let salSesudah = salSebelum + targetNominal;

                        p.topup.status = 'Sukses'; p.topup.saldo_sebelum = salSebelum; p.topup.saldo_sesudah = salSesudah;
                        ...
                        if(db[p.phone].mutasi.length > 50) db[p.phone].mutasi = db[p.phone].mutasi.slice(-50);
                        ...
                        let uName = uData.name || 'Hamba Allah'; let uEmail = uData.email || 'Belum diatur';
...
function startAutoBackup() {
    ...
    setInterval(() => {
        ...
        exec(`zip -r ${zipName} database.json web_users.json config.json local_products.json info.json public/banners public/info_images`, async (err) => {
            if (!err) { 
                const form = new FormData(); form.append('chat_id', config.teleChatAdmin); form.append('caption', `⏳ *AUTO BACKUP SYSTEM (${config.autoBackupHours} Jam)*\n\nTanggal: ${new Date().toLocaleString('id-ID', {timeZone: 'Asia/Jakarta'})}`); form.append('document', fs.createReadStream(zipName)); form.append('parse_mode', 'Markdown');
                try { await axios.post(`https://api.telegram.org/bot${config.teleTokenAdmin}/sendDocument`, form, { headers: form.getHeaders() }); } catch(e) {} fs.unlinkSync(zipName); 
            }
        });
...
// ==========================================
// API TOPUP LOGIC (FITUR ANTI-SPAM & SAVE QRIS)
// ==========================================
app.post('/api/topup/request', (req, res) => {
    ...
    // ANTI SPAM LOGIC
    let hasPending = db[phone].topup.some(t => t.status === 'Proses');
    if (hasPending) return res.status(400).json({ error: '❌ Anda masih memiliki Top Up yang berstatus PROSES. Harap selesaikan pembayarannya atau tunggu hingga batas waktu kedaluwarsa sebelum membuat Top Up baru.' });
    
    let expiry = null; let finalQrisString = null;
    if (method === 'QRIS Otomatis') {
        expiry = Date.now() + 10 * 60 * 1000; 
        if (config.qrisStringCode && config.qrisStringCode !== '') { finalQrisString = generateDynamicQris(config.qrisStringCode, nominal); } else { return res.status(400).json({ error: 'Admin belum menyetting String QRIS. Hubungi Admin.' }); }
    }

    let dateStr = new Date().toLocaleString('id-ID', {timeZone: 'Asia/Jakarta'});
    let newRef = 'TP-' + Date.now(); let salSebelum = db[phone].saldo; let kodeUnik = nominal % 1000; let depositAsli = nominal - kodeUnik;
    
    const newTopup = { id: newRef, method: method, nominal: nominal, status: 'Proses', date: dateStr, expiry: expiry, saldo_sebelum: salSebelum, kode_unik: kodeUnik, nama: uData.name, email: uData.email, wa: phone, qris_string: finalQrisString };
    
    db[phone].topup.push(newTopup); 
    if(db[phone].topup.length > 50) db[phone].topup = db[phone].topup.slice(-50); // LIMIT 50 TRANSAKSI
    saveJSON(dbFile, db); 
...
app.get('/api/admin/backup', async (req, res) => {
    let config = loadJSON(configFile); if (!config.teleTokenAdmin || !config.teleChatAdmin) return res.status(400).json({ error: "Token/Chat ID Admin belum disetting." });
    let zipName = `ManualBackup_FikyStore_${Date.now()}.zip`;
    exec(`zip -r ${zipName} database.json web_users.json config.json local_products.json info.json public/banners public/info_images`, async (err) => {
        if(err) return res.status(500).json({ error: "Gagal membuat file ZIP." });
        const form = new FormData(); form.append('chat_id', config.teleChatAdmin); form.append('caption', `📦 *BACKUP MANUAL FULL SYSTEM*`); form.append('document', fs.createReadStream(zipName));
        try { await axios.post(`https://api.telegram.org/bot${config.teleTokenAdmin}/sendDocument`, form, { headers: form.getHeaders() }); fs.unlinkSync(zipName); res.json({ message: "Backup sukses!" }); } catch(e) { res.status(500).json({ error: "Gagal mengirim ke Telegram." }); }
    });
});
...
app.post('/api/admin/balance', async (req, res) => {
    ...
    if (action === 'add') { 
        let nominalLengkap = parseInt(amount); let salSesudah = salSebelum + nominalLengkap; db[targetPhone].saldo = salSesudah; 
        db[targetPhone].mutasi.push({ id: 'TRX'+Date.now(), type: 'in', amount: nominalLengkap, desc: 'Penambahan oleh Admin', date: dateStr }); 
        db[targetPhone].topup.push({ id: 'TU'+Date.now(), method: 'Admin Fiky Store', nominal: nominalLengkap, status: 'Sukses', date: dateStr, saldo_sebelum: salSebelum, saldo_sesudah: salSesudah, kode_unik: 0, nama: webUsers[targetPhone].name, email: webUsers[targetPhone].email, wa: targetPhone }); 
        if(db[targetPhone].mutasi.length > 50) db[targetPhone].mutasi = db[targetPhone].mutasi.slice(-50);
        if(db[targetPhone].topup.length > 50) db[targetPhone].topup = db[targetPhone].topup.slice(-50);
        saveJSON(dbFile, db); 
        ...
        db[targetPhone].saldo -= parseInt(amount); db[targetPhone].mutasi.push({ id: 'TRX'+Date.now(), type: 'out', amount: parseInt(amount), desc: 'Penarikan oleh Admin', date: dateStr }); saveJSON(dbFile, db); res.json({ success: true, message: `\n✅ Saldo berhasil dikurangi!` }); 
    }
});
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try { await global.waSocket?.sendMessage(fPhone + '@c.us', { text: `Halo kak *${name}* 👋\n\nTerima kasih telah mendaftar di *DIGITAL FIKY STORE* 👑\n\nKode OTP Anda:\n\n*${otp}*\n\n⏳ _Berlaku selama 5 menit._` }); res.json({ message: 'OTP Terkirim', phone: fPhone }); } catch(e) { res.status(500).json({ error: 'Gagal kirim WA. Pastikan bot di VPS jalan.' }); }
});
...
app.post('/api/auth/update', (req, res) => {
    ...
    if (isSecureChange) {
        ...
        if (fOld !== fNew) { 
            webUsers[fNew] = { ...webUsers[fOld], name: newName, avatar: avatar || webUsers[fOld].avatar }; if (newPassword) webUsers[fNew].password = newPassword; delete webUsers[fNew].updateOtp; delete webUsers[fNew].updateOtpExpiry; delete webUsers[fOld]; 
...
echo "[9/10] Membangun Panel Manajemen VPS Terlengkap..."
    ...
    case $choice in
        ...
        7)
            ...
            if [ "$s_menu" == "1" ]; then
                ...
                read -p "No WA Member/Email: " no_mem
...
echo -e "  ✅ \033[1;33mANTI-SPAM TOP UP & TRANSAKSI LIMIT\033[0m Max 50 data, cegah DB Lemot!"
echo -e "  ✅ \033[1;33mAUTO MAINTENANCE ALARM\033[0m 22:50 otomatis broadcast notif maintenance."
echo -e "  ✅ \033[1;33mSTATISTIK SINKRON\033[0m Penjualan harian & mingguan 100% akurat!"
echo -e "  ✅ \033[1;33mPISAH NOTIF & SENSOR 'xxx'\033[0m Tele aman, bot WA saluran hanya sukses."
echo -e "  ✅ \033[1;33mSAVE QRIS DI RIWAYAT\033[0m QRIS tampil paling atas & sisa timer live."
echo -e "  ✅ \033[1;33mAUTO MENU VPS LOGIN\033[0m Langsung muncul panel saat SSH login."
echo -e "\033[0;32m======================================================================\033[0m"
