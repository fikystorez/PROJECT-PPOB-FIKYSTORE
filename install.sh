...
<body class="bg-gray-50 dark:bg-[#0b1320] font-sans transition-colors duration-300">
    ...
    <script>
        ...
        const plnCategories = {
            'pln_token': { name: 'Token PLN', logo: 'fas fa-bolt', isIcon: true, placeholder: 'Masukkan No Meter / ID Pelanggan PLN...', items: ['Token PLN 20.000', 'Token PLN 50.000', 'Token PLN 100.000', 'Token PLN 200.000', 'Token PLN 500.000', 'Token PLN 1.000.000'] },
            'pln_tagihan': { name: 'Tagihan PLN', logo: 'fas fa-file-invoice-dollar', isIcon: true, placeholder: 'Masukkan ID Pelanggan PLN...', items: ['Cek & Bayar Tagihan PLN Pascabayar'] }
        };

        const allProviders = {};
        Object.assign(allProviders, dataCategories, ewalletCategories, etollCategories, plnCategories);
        ...
        if(type === 'ewallet') {
            ...
            originalTitle = 'PLN';
            subtitleEl.innerText = 'PILIH JENIS LAYANAN PLN';
            currentList = plnCategories;
            ...
            for(let key in currentList){
                ...
                opHtml += `
                    <div class="flex items-center p-4 border-b border-gray-200 dark:border-gray-800 cursor-pointer hover:bg-gray-50 dark:hover:bg-[#1a2639] transition" onclick="selectProvider('${key}')">
                        <div class="w-10 h-10 rounded-full border border-black dark:border-gray-400 flex items-center justify-center text-black dark:text-gray-300 font-bold text-[10px] shrink-0 text-center leading-tight bg-white dark:bg-[#0b1320]">${op.isIcon ? `<i class="${op.logo} text-lg"></i>` : op.logo}</div>
            ...
            if(provider) {
                ...
                if (provider.items && provider.items.length > 0) {
                    ...
                    document.getElementById('categoryList').innerHTML = `
                        <div class="py-12 flex flex-col items-center justify-center text-gray-400 dark:text-gray-600 transition-colors">
                            <i class="fas fa-box-open text-5xl mb-4 opacity-40"></i>
                            <p class="text-[11px] font-bold tracking-wide uppercase">Katalog Produk Kosong</p>
                        </div>
                    `;
                }
            } else {
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}           💎 PANEL DIGITAL FIKY STORE (V81) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
