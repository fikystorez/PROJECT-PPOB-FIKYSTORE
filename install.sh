...
# ==========================================
# FILE NODE.JS (API) + AUTO-HEAL DATABASE
# ==========================================
...
// API TRANSAKSI (V93: ANTI-CRASH & AUTO-HEAL)
    ...
    try {
        ...
        if (!isLocal && config.digiUser && config.digiKey) {
            try {
                ...
                // DI V93 KITA PASTIKAN testing: false JIKA MENGGUNAKAN PROD KEY
                let isDev = (config.digiKey || '').toLowerCase().startsWith('dev');

                console.log(`[DIGIFLAZZ] Mengirim TRX: ${sku} ke ${target} (Dev Mode: ${isDev})`);
                ...
                console.log(`[DIGIFLAZZ] Response Status:`, digiData.status);
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}           💎 PANEL DIGITAL FIKY STORE (V93) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
...
EOFNGINX
        ...
        13)
            ...
            read -p "Masukkan API Key (WAJIB PRODUCTION KEY jika ingin potong saldo asli): " digi_key
