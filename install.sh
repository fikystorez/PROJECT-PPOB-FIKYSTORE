...
// DIGIFLAZZ CACHE & MARKUP BERTINGKAT
...
app.post('/api/products', async (req, res) => {
    ...
    try {
        ...
        // LOGIKA MARKUP BERTINGKAT
        let markupTiers = config.markupTiers || { 
            tier1: parseInt(config.markup)||0, 
            tier2: parseInt(config.markup)||0, 
            tier3: parseInt(config.markup)||0, 
            tier4: parseInt(config.markup)||0 
        };
...
while true; do clear
    echo -e "${CYAN}======================================================${NC}"
    echo -e "${YELLOW}           💎 PANEL DIGITAL FIKY STORE (V85) 💎       ${NC}"
    echo -e "${CYAN}======================================================${NC}"
    echo ""
...
EOFNGINX
        ...
        12)
            ...
            echo -e "\n${YELLOW}--- SETTING KEUNTUNGAN (MARKUP) BERTINGKAT ---${NC}"
            ...
            if [ ! -z "$digi_user" ] && [ ! -z "$digi_key" ]; then
                node -e "const fs=require('fs');let file='$HOME/$DIR_NAME/config.json';let cfg=fs.existsSync(file)?JSON.parse(fs.readFileSync(file)):{};cfg.digiUser='$digi_user';cfg.digiKey='$digi_key';cfg.markupTiers={tier1:parseInt('$m1')||0,tier2:parseInt('$m2')||0,tier3:parseInt('$m3')||0,tier4:parseInt('$m4')||0};fs.writeFileSync(file,JSON.stringify(cfg,null,2));console.log('\n✅ Data API Digiflazz & Markup Bertingkat berhasil disimpan!');"
